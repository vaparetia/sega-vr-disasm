/* math/matrix.c — 16.16 fixed-point 4×4 matrix operations.
 *
 * Sin table: 512-step full circle, 16.16 fixed-point.
 * angle 0 → 0, angle 128 → 65536 (1.0), angle 256 → 0, angle 384 → -65536.
 * Precomputed from sin(i * 2π / 512) * 65536, rounded to nearest integer.
 */

#include "matrix.h"
#include <string.h>

/* 512-entry sin table, 16.16 fixed-point. */
static const int32_t s_sin_table[512] = {
...
{
        /* Table is precomputed — no runtime init needed. */
}

fp16_t mat_sin(int a)
{
        return s_sin_table[(unsigned)a & 511u];
}

fp16_t mat_cos(int a)
{
        return s_sin_table[((unsigned)a + 128u) & 511u];
}
...
{
        *ox = fp_mul(m->m[0][0], x) + fp_mul(m->m[0][1], y)
            + fp_mul(m->m[0][2], z) + m->m[0][3];
        *oy = fp_mul(m->m[1][0], x) + fp_mul(m->m[1][1], y)
            + fp_mul(m->m[1][2], z) + m->m[1][3];
        *oz = fp_mul(m->m[2][0], x) + fp_mul(m->m[2][1], y)
            + fp_mul(m->m[2][2], z) + m->m[2][3];
}
