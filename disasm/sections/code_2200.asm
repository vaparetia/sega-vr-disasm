; ============================================================================
; Code Section ($002200-$0041FF)
; ============================================================================

        org     $002200

code_2200:
        dc.w    $5238        ; $002200
        dc.w    $C828        ; $002202
        dc.w    $08F8        ; $002204
        dc.w    $0001        ; $002206
        dc.w    $C80B        ; $002208
        dc.w    $4E75        ; $00220A
        dc.w    $3038        ; $00220C
        dc.w    $9F74        ; $00220E
        dc.w    $1238        ; $002210
        dc.w    $9FE5        ; $002212
        dc.w    $47F8        ; $002214
        dc.w    $8759        ; $002216
        dc.w    $45F8        ; $002218
        dc.w    $8517        ; $00221A
        dc.w    $43F8        ; $00221C
        dc.w    $8760        ; $00221E
        dc.w    $6114        ; $002220
        dc.w    $3038        ; $002222
        dc.w    $9074        ; $002224
        dc.w    $1238        ; $002226
        dc.w    $90E5        ; $002228
        dc.w    $47F8        ; $00222A
        dc.w    $8789        ; $00222C
        dc.w    $45F8        ; $00222E
        dc.w    $8516        ; $002230
        dc.w    $43F8        ; $002232
        dc.w    $8790        ; $002234

; --- Bit test and branch utility ---
BitTestAndBranch:
        BTST #4,D1        ; $002236
        BEQ $0088224C        ; $00223A
        MOVE.B $C827.W,D1        ; $00223C
        CMP.B (A3),D1        ; $002240
        BEQ $0088225A        ; $002242
        MOVE.B D1,(A3)        ; $002244
        MOVE.B #$0001,(A2)        ; $002246
        BRA $0088225A        ; $00224A
        MOVE.B $C828.W,D1        ; $00224C
        CMP.B (A3),D1        ; $002250
        BEQ $0088225A        ; $002252
        MOVE.B D1,(A3)        ; $002254
        MOVE.B #$0001,(A2)        ; $002256
        CMPI.W #$0000,$C8C8.W        ; $00225A
        BEQ $008822AA        ; $002260
        CMPI.W #$0002,$C8C8.W        ; $002262
        BEQ $008822EC        ; $002268
        LSR.W #5,D0        ; $00226C
        MOVE.W D0,D1        ; $00226E
        LSR.W #2,D0        ; $002270
        ADD.W D0,D1        ; $002272
        LSR.W #1,D0        ; $002274
        ADD.W D0,D1        ; $002276
        BCHG #94,D1        ; $002278
        ADD.W (A1),D1        ; $00227C
        LSR.W #1,D1        ; $00227E
        CMPI.W #$1E00,D1        ; $002280
        BGT $00882294        ; $002284
        CMPI.W #$1A5E,D1        ; $002286
        BGT $00882298        ; $00228A
        MOVE.W #$1A5E,D1        ; $00228C
        MOVE.W D1,(A1)        ; $002290
        RTS        ; $002292
        dc.w    $323C        ; $002294
        dc.w    $1E00        ; $002296
        dc.w    $B251        ; $002298
        dc.w    $660A        ; $00229A
        dc.w    $4EBA        ; $00229C
        dc.w    $26D0        ; $00229E
        dc.w    $0240        ; $0022A0
        dc.w    $000F        ; $0022A2
        dc.w    $9240        ; $0022A4
        dc.w    $3281        ; $0022A6
        dc.w    $4E75        ; $0022A8
        dc.w    $E848        ; $0022AA
        dc.w    $3200        ; $0022AC
        dc.w    $E248        ; $0022AE
        dc.w    $D240        ; $0022B0
        dc.w    $E248        ; $0022B2
        dc.w    $D240        ; $0022B4
        dc.w    $E448        ; $0022B6
        dc.w    $D240        ; $0022B8
        dc.w    $0641        ; $0022BA
        dc.w    $1A5E        ; $0022BC
        dc.w    $D251        ; $0022BE
        dc.w    $E249        ; $0022C0
        dc.w    $0C41        ; $0022C2
        dc.w    $21D0        ; $0022C4
        dc.w    $6E0E        ; $0022C6
        dc.w    $0C41        ; $0022C8
        dc.w    $1A5E        ; $0022CA
        dc.w    $6E0C        ; $0022CC
        dc.w    $323C        ; $0022CE
        dc.w    $1A5E        ; $0022D0
        dc.w    $3281        ; $0022D2
        dc.w    $4E75        ; $0022D4
        dc.w    $323C        ; $0022D6
        dc.w    $21D0        ; $0022D8
        dc.w    $B251        ; $0022DA
        dc.w    $660A        ; $0022DC
        dc.w    $4EBA        ; $0022DE
        dc.w    $268E        ; $0022E0
        dc.w    $0240        ; $0022E2
        dc.w    $000F        ; $0022E4
        dc.w    $9240        ; $0022E6
        dc.w    $3281        ; $0022E8
        dc.w    $4E75        ; $0022EA
        dc.w    $E848        ; $0022EC
        dc.w    $3200        ; $0022EE
        dc.w    $E248        ; $0022F0
        dc.w    $D240        ; $0022F2
        dc.w    $E248        ; $0022F4
        dc.w    $D240        ; $0022F6
        dc.w    $0641        ; $0022F8
        dc.w    $1A5E        ; $0022FA
        dc.w    $D251        ; $0022FC
        dc.w    $E249        ; $0022FE
        dc.w    $0C41        ; $002300
        dc.w    $21A0        ; $002302
        dc.w    $6E0E        ; $002304
        dc.w    $0C41        ; $002306
        dc.w    $1A5E        ; $002308
        dc.w    $6E0C        ; $00230A
        dc.w    $323C        ; $00230C
        dc.w    $1A5E        ; $00230E
        dc.w    $3281        ; $002310
        dc.w    $4E75        ; $002312
        dc.w    $323C        ; $002314
        dc.w    $21A0        ; $002316
        dc.w    $B251        ; $002318
        dc.w    $660A        ; $00231A
        dc.w    $4EBA        ; $00231C
        dc.w    $2650        ; $00231E
        dc.w    $0240        ; $002320
        dc.w    $000F        ; $002322
        dc.w    $9240        ; $002324
        dc.w    $3281        ; $002326
        dc.w    $4E75        ; $002328
        dc.w    $AFAD        ; $00232A
        dc.w    $AE00        ; $00232C
        dc.w    $3038        ; $00232E
        dc.w    $9074        ; $002330
        dc.w    $43F8        ; $002332
        dc.w    $8790        ; $002334
        dc.w    $0838        ; $002336
        dc.w    $0004        ; $002338
        dc.w    $90E5        ; $00233A
        dc.w    $671A        ; $00233C
        dc.w    $0C38        ; $00233E
        dc.w    $0001        ; $002340
        dc.w    $C823        ; $002342
        dc.w    $6724        ; $002344
        dc.w    $11FC        ; $002346
        dc.w    $0001        ; $002348
        dc.w    $C823        ; $00234A
        dc.w    $3038        ; $00234C
        dc.w    $C8C8        ; $00234E
        dc.w    $11FB        ; $002350
        dc.w    $00D8        ; $002352
        dc.w    $C8A5        ; $002354
        dc.w    $6012        ; $002356
        dc.w    $4A38        ; $002358
        dc.w    $C823        ; $00235A
        dc.w    $670C        ; $00235C
        dc.w    $11FC        ; $00235E
        dc.w    $0000        ; $002360
        dc.w    $C823        ; $002362
        dc.w    $11FC        ; $002364
        dc.w    $00AB        ; $002366
        dc.w    $C8A5        ; $002368
        dc.w    $0838        ; $00236A
        dc.w    $0001        ; $00236C
        dc.w    $C80B        ; $00236E
        dc.w    $6712        ; $002370
        dc.w    $11F8        ; $002372
        dc.w    $C828        ; $002374
        dc.w    $8789        ; $002376
        dc.w    $11FC        ; $002378
        dc.w    $0001        ; $00237A
        dc.w    $8516        ; $00237C
        dc.w    $08B8        ; $00237E
        dc.w    $0001        ; $002380
        dc.w    $C80B        ; $002382
        dc.w    $0C78        ; $002384
        dc.w    $0000        ; $002386
        dc.w    $C8C8        ; $002388
        dc.w    $6750        ; $00238A
        dc.w    $0C78        ; $00238C
        dc.w    $0002        ; $00238E
        dc.w    $C8C8        ; $002390
        dc.w    $6700        ; $002392
        dc.w    $0092        ; $002394
        dc.w    $EA48        ; $002396
        dc.w    $3200        ; $002398
        dc.w    $E448        ; $00239A
        dc.w    $D240        ; $00239C
        dc.w    $E248        ; $00239E
        dc.w    $D240        ; $0023A0
        dc.w    $0641        ; $0023A2
        dc.w    $1A5E        ; $0023A4
        dc.w    $D251        ; $0023A6
        dc.w    $E249        ; $0023A8
        dc.w    $0C41        ; $0023AA
        dc.w    $1E00        ; $0023AC
        dc.w    $6E12        ; $0023AE
        dc.w    $0C41        ; $0023B0
        dc.w    $1A5E        ; $0023B2
        dc.w    $6E10        ; $0023B4
        dc.w    $323C        ; $0023B6
        dc.w    $1A5E        ; $0023B8
        dc.w    $3281        ; $0023BA
        dc.w    $31D1        ; $0023BC
        dc.w    $8760        ; $0023BE
        dc.w    $4E75        ; $0023C0
        dc.w    $323C        ; $0023C2
        dc.w    $1E00        ; $0023C4
        dc.w    $B251        ; $0023C6
        dc.w    $660A        ; $0023C8
        dc.w    $4EBA        ; $0023CA
        dc.w    $25A2        ; $0023CC
        dc.w    $0240        ; $0023CE
        dc.w    $000F        ; $0023D0
        dc.w    $9240        ; $0023D2
        dc.w    $3281        ; $0023D4
        dc.w    $31D1        ; $0023D6
        dc.w    $8760        ; $0023D8
        dc.w    $4E75        ; $0023DA
        dc.w    $E848        ; $0023DC
        dc.w    $3200        ; $0023DE
        dc.w    $E248        ; $0023E0
        dc.w    $D240        ; $0023E2
        dc.w    $E248        ; $0023E4
        dc.w    $D240        ; $0023E6
        dc.w    $E448        ; $0023E8
        dc.w    $D240        ; $0023EA
        dc.w    $0641        ; $0023EC
        dc.w    $1A5E        ; $0023EE
        dc.w    $D251        ; $0023F0
        dc.w    $E249        ; $0023F2
        dc.w    $0C41        ; $0023F4
        dc.w    $21D0        ; $0023F6
        dc.w    $6E12        ; $0023F8
        dc.w    $0C41        ; $0023FA
        dc.w    $1A5E        ; $0023FC
        dc.w    $6E10        ; $0023FE
        dc.w    $323C        ; $002400
        dc.w    $1A5E        ; $002402
        dc.w    $3281        ; $002404
        dc.w    $31D1        ; $002406
        dc.w    $8760        ; $002408
        dc.w    $4E75        ; $00240A
        dc.w    $323C        ; $00240C
        dc.w    $21D0        ; $00240E
        dc.w    $B251        ; $002410
        dc.w    $660A        ; $002412
        dc.w    $4EBA        ; $002414
        dc.w    $2558        ; $002416
        dc.w    $0240        ; $002418
        dc.w    $000F        ; $00241A
        dc.w    $9240        ; $00241C
        dc.w    $3281        ; $00241E
        dc.w    $31D1        ; $002420
        dc.w    $8760        ; $002422
        dc.w    $4E75        ; $002424
        dc.w    $E848        ; $002426
        dc.w    $3200        ; $002428
        dc.w    $E248        ; $00242A
        dc.w    $D240        ; $00242C
        dc.w    $E248        ; $00242E
        dc.w    $D240        ; $002430
        dc.w    $0641        ; $002432
        dc.w    $1A5E        ; $002434
        dc.w    $D251        ; $002436
        dc.w    $E249        ; $002438
        dc.w    $0C41        ; $00243A
        dc.w    $21A0        ; $00243C
        dc.w    $6E12        ; $00243E
        dc.w    $0C41        ; $002440
        dc.w    $1A5E        ; $002442
        dc.w    $6E10        ; $002444
        dc.w    $323C        ; $002446
        dc.w    $1A5E        ; $002448
        dc.w    $3281        ; $00244A
        dc.w    $31D1        ; $00244C
        dc.w    $8760        ; $00244E
        dc.w    $4E75        ; $002450
        dc.w    $323C        ; $002452
        dc.w    $21A0        ; $002454
        dc.w    $B251        ; $002456
        dc.w    $660A        ; $002458
        dc.w    $4EBA        ; $00245A
        dc.w    $2512        ; $00245C
        dc.w    $0240        ; $00245E
        dc.w    $000F        ; $002460
        dc.w    $9240        ; $002462
        dc.w    $3281        ; $002464
        dc.w    $31D1        ; $002466
        dc.w    $8760        ; $002468
        dc.w    $4E75        ; $00246A
        dc.w    $11FC        ; $00246C
        dc.w    $0001        ; $00246E
        dc.w    $8507        ; $002470
        dc.w    $4E75        ; $002472
        dc.w    $11FC        ; $002474
        dc.w    $0080        ; $002476
        dc.w    $8507        ; $002478
        dc.w    $4E75        ; $00247A
        dc.w    $3C3C        ; $00247C
        dc.w    $E001        ; $00247E
        dc.w    $7000        ; $002480
        dc.w    $7200        ; $002482
        dc.w    $1018        ; $002484
        dc.w    $1200        ; $002486
        dc.w    $E808        ; $002488
        dc.w    $0201        ; $00248A
        dc.w    $000F        ; $00248C
        dc.w    $D046        ; $00248E
        dc.w    $D246        ; $002490
        dc.w    $3C80        ; $002492
        dc.w    $3C81        ; $002494
        dc.w    $7000        ; $002496
        dc.w    $7200        ; $002498
        dc.w    $1018        ; $00249A
        dc.w    $1200        ; $00249C
        dc.w    $E808        ; $00249E
        dc.w    $0201        ; $0024A0
        dc.w    $000F        ; $0024A2
        dc.w    $D046        ; $0024A4
        dc.w    $D246        ; $0024A6
        dc.w    $3C80        ; $0024A8
        dc.w    $3C81        ; $0024AA
        dc.w    $4E75        ; $0024AC
        dc.w    $3C3C        ; $0024AE
        dc.w    $E001        ; $0024B0
        dc.w    $7000        ; $0024B2
        dc.w    $7200        ; $0024B4
        dc.w    $1018        ; $0024B6
        dc.w    $1200        ; $0024B8
        dc.w    $E808        ; $0024BA
        dc.w    $0201        ; $0024BC
        dc.w    $000F        ; $0024BE
        dc.w    $D046        ; $0024C0
        dc.w    $D246        ; $0024C2
        dc.w    $3C80        ; $0024C4
        dc.w    $3C81        ; $0024C6
        dc.w    $4E75        ; $0024C8

; --- Data processing utility ---
DataProcessing1:
        TST.B $C80D.W        ; $0024CA
        BNE $00882592        ; $0024CE
        MOVE.W #$0000,$8000.W        ; $0024D2
        MOVE.W #$FFF8,$C880.W        ; $0024D8
        LEA $00FF6116,A0        ; $0024DE
        MOVE.L #$62020002,(A5)        ; $0024E4
        JSR $0088247C(PC)        ; $0024EA
        DC.W $41F8 ; Unknown        ; $0024EE
        SUB.B <EA:32>,D0        ; $0024F0
        MOVE.L #$620C0002,(A5)        ; $0024F2
        JSR $0088247C(PC)        ; $0024F8

; --- Data transformation ---
DataTransform:
        DC.W $FF82 ; Unknown        ; $0024FA
        LEA $00FF611A,A0        ; $0024FC
        MOVE.L #$62160002,(A5)        ; $002502
        JSR $0088247C(PC)        ; $002508
        LEA $00FF6108,A0        ; $00250C
        MOVE.L #$63020002,(A5)        ; $002512
        JSR $0088247C(PC)        ; $002518

; --- Memory initialization 1 ---
MemoryInit1:
        DC.W $FF62 ; Unknown        ; $00251A

; --- Memory operation 1 ---
MemoryOp1:
        LEA $00FF610A,A0        ; $00251C
        MOVE.L #$630C0002,(A5)        ; $002522
        JSR $0088247C(PC)        ; $002528

; --- Data processing 2 ---
DataProcessing2:
        LEA $00FF610C,A0        ; $00252C
        MOVE.L #$63160002,(A5)        ; $002532
        JSR $0088247C(PC)        ; $002538
        LEA $00FF6104,A0        ; $00253C

; --- Utility operation 1 ---
UtilityOp1:
        BSET #4,<EA:3F>        ; $00253E
        MOVE.L #$632A0002,(A5)        ; $002542

; --- Data handling routine ---
DataHandling:
        BTST #186,D2        ; $002546
        DC.W $FF32 ; Unknown        ; $00254A
        LEA $00FF6106,A0        ; $00254C
        MOVE.L #$63340002,(A5)        ; $002552

; --- Memory utility ---
MemoryUtil:
        JSR $0088247C(PC)        ; $002558
        LEA $00FF5FF8,A0        ; $00255C
        MOVE.L #$640C0002,(A5)        ; $002562
        JSR $0088247C(PC)        ; $002568
        MOVE.L #$64160002,(A5)        ; $00256C
        JSR $0088247C(PC)        ; $002572
        MOVE.L #$64200002,(A5)        ; $002576
        JSR $0088247C(PC)        ; $00257C
        MOVE.L #$642A0002,(A5)        ; $002580
        JSR $0088247C(PC)        ; $002586
        MOVE.B #$0000,$00FF5FFF        ; $00258A
        RTS        ; $002592
        dc.w    $4A38        ; $002594
        dc.w    $C80D        ; $002596
        dc.w    $6614        ; $002598
        dc.w    $41F8        ; $00259A
        dc.w    $C886        ; $00259C
        dc.w    $2ABC        ; $00259E
        dc.w    $622A        ; $0025A0
        dc.w    $0002        ; $0025A2
        dc.w    $4EBA        ; $0025A4
        dc.w    $FF08        ; $0025A6
        dc.w    $11FC        ; $0025A8
        dc.w    $0000        ; $0025AA
        dc.w    $C886        ; $0025AC
        dc.w    $4E75        ; $0025AE

; --- Memory operation 2 ---
MemoryOp2:
        TST.B $C80D.W        ; $0025B0
        BNE $0088263C        ; $0025B4
        DC.W $41F8 ; Unknown        ; $0025B8
        AND.L A0,D4        ; $0025BA
        MOVE.L #$65020002,(A5)        ; $0025BC
        JSR $0088247C(PC)        ; $0025C2
        JSR $0088247C(PC)        ; $0025C6
        MOVEA.L $C888.W,A0        ; $0025CA
        MOVE.L #$65140002,(A5)        ; $0025CE
        JSR $0088247C(PC)        ; $0025D4
        MOVE.L #$651E0002,(A5)        ; $0025D8
        JSR $0088247C(PC)        ; $0025DE
        MOVE.L #$65280002,(A5)        ; $0025E2
        JSR $0088247C(PC)        ; $0025E8
        MOVE.L #$65320002,(A5)        ; $0025EC
        JSR $0088247C(PC)        ; $0025F2
        SUBQ.L #8,$C888.W        ; $0025F6
        DC.W $41F8 ; Unknown        ; $0025FA
        AND.L A0,D4        ; $0025FC
        MOVE.L #$66020002,(A5)        ; $0025FE
        JSR $0088247C(PC)        ; $002604
        JSR $0088247C(PC)        ; $002608
        MOVEA.L $C888.W,A0        ; $00260C
        MOVE.L #$66140002,(A5)        ; $002610
        JSR $0088247C(PC)        ; $002616
        MOVE.L #$661E0002,(A5)        ; $00261A
        JSR $0088247C(PC)        ; $002620
        MOVE.L #$66280002,(A5)        ; $002624
        JSR $0088247C(PC)        ; $00262A
        MOVE.L #$66320002,(A5)        ; $00262E
        JSR $0088247C(PC)        ; $002634
        DC.W $51B8 ; Unknown        ; $002638
        AND.L A0,D4        ; $00263A
        RTS        ; $00263C
        dc.w    $43FA        ; $00263E
        dc.w    $0012        ; $002640
        dc.w    $45F9        ; $002642
        dc.w    $00A1        ; $002644
        dc.w    $5100        ; $002646
        dc.w    $7E0C        ; $002648
        dc.w    $34D9        ; $00264A
        dc.w    $51CF        ; $00264C
        dc.w    $FFFC        ; $00264E
        dc.w    $4E75        ; $002650
        dc.w    $0083        ; $002652
        dc.w    $0000        ; $002654
        dc.w    $0001        ; $002656
        dc.w    $0000        ; $002658
        dc.w    $0000        ; $00265A
        dc.w    $0000        ; $00265C
        dc.w    $0000        ; $00265E
        dc.w    $0000        ; $002660
        dc.w    $0000        ; $002662
        dc.w    $0000        ; $002664
        dc.w    $0000        ; $002666
        dc.w    $0000        ; $002668
        dc.w    $0000        ; $00266A

; --- VDP-related operation 1 ---
VDPOp1:
        LEA $00882680(PC),A1        ; $00266C
        LEA MARS_VDP_MODE,A2        ; $002670
        MOVEQ #$05,D7        ; $002676
        MOVE.W (A1)+,(A2)+        ; $002678
        DBRA D7,$00882678        ; $00267A
        RTS        ; $00267E
        dc.w    $8000        ; $002680
        dc.w    $0000        ; $002682
        dc.w    $0000        ; $002684
        dc.w    $0000        ; $002686
        dc.w    $0000        ; $002688
        dc.w    $0000        ; $00268A

; --- VDP-related operation 2 ---
VDPOp2:
        LEA MARS_SYS_BASE,A4        ; $00268C
        BCLR #0,$008B(A4)        ; $002692
        JSR $00882742(PC)        ; $002698
        JSR $008827BE(PC)        ; $00269C
        JSR $0088281E(PC)        ; $0026A0
        BSET #0,$008B(A4)        ; $0026A4
        JSR $00882742(PC)        ; $0026AA
        JSR $008827BE(PC)        ; $0026AE
        JSR $0088281E(PC)        ; $0026B2
        BCLR #0,$008B(A4)        ; $0026B6
        MOVE.B #$0000,$C80C.W        ; $0026BC
        MOVEQ #$00,D0        ; $0026C2
        DC.W $4EFA ; Unknown        ; $0026C4
        BCLR #249,#$00A1        ; $0026C6

; --- Frame buffer FM toggle (10 calls) ---
VDPFrameControl:
        LEA MARS_SYS_BASE,A4        ; $0026C8
        BCLR #0,$008B(A4)        ; $0026CE
        BSR $00882742        ; $0026D4
        JSR $008827A0(PC)        ; $0026D6
        JSR $0088281E(PC)        ; $0026DA
        BSET #0,$008B(A4)        ; $0026DE
        BSR $00882742        ; $0026E4
        JSR $008827A0(PC)        ; $0026E6
        JSR $0088281E(PC)        ; $0026EA
        BCLR #0,$008B(A4)        ; $0026EE
        MOVE.B #$0000,$C80C.W        ; $0026F4
        MOVEQ #$00,D0        ; $0026FA
        JSR $00882782(PC)        ; $0026FC
        MOVE.W #$8000,$00A15202        ; $002700
        RTS        ; $002708

; --- VDP operation 3 ---
VDPOp3:
        LEA MARS_SYS_BASE,A4        ; $00270A
        BSR $00882742        ; $002710
        JSR $008827A0(PC)        ; $002712
        JSR $008827DA(PC)        ; $002716
        MOVEQ #$01,D0        ; $00271A
        MOVEQ #$00,D2        ; $00271C
        BTST #0,$C80C.W        ; $00271E
        BEQ $0088272A        ; $002724
        MOVEQ #$00,D0        ; $002726
        MOVEQ #$01,D2        ; $002728
        MOVE.B D0,$008B(A4)        ; $00272A
        BSR $00882742        ; $00272E
        BSR $008827A0        ; $002730
        JSR $008827DA(PC)        ; $002732
        MOVE.B D2,$008B(A4)        ; $002736
        RTS        ; $00273A
        dc.w    $49F9        ; $00273C
        dc.w    $00A1        ; $00273E
        dc.w    $5100        ; $002740

; --- VDP auto-fill operation ---
VDPAutoFill:
        LEA MARS_VDP_FILLADR,A2        ; $002742
        LEA MARS_VDP_FILLDATA,A3        ; $002748
        BTST #64,$0081(A4)        ; $00274E
        MOVE.W #$00FF,D7        ; $002754
        MOVEQ #$00,D0        ; $002758
        MOVEQ #$00,D1        ; $00275A
        MOVE.W #$0100,D2        ; $00275C
        MOVE.W #$00FF,$0084(A4)        ; $002760
        MOVE.W D1,(A2)        ; $002766
        MOVE.W D0,(A3)        ; $002768
        BTST #1,$008B(A4)        ; $00276A
        BNE $0088276A        ; $002770
        ADD.W D2,D1        ; $002772
        DBRA D7,$00882766        ; $002774
        RTS        ; $002778
        dc.w    $0239        ; $00277A
        dc.w    $0040        ; $00277C
        dc.w    $00A1        ; $00277E
        dc.w    $5181        ; $002780
        dc.w    $45F9        ; $002782
        dc.w    $00A1        ; $002784
        dc.w    $5200        ; $002786
        dc.w    $7E1F        ; $002788
        dc.w    $24C0        ; $00278A
        dc.w    $24C0        ; $00278C
        dc.w    $24C0        ; $00278E
        dc.w    $24C0        ; $002790
        dc.w    $51CF        ; $002792
        dc.w    $FFF6        ; $002794
        dc.w    $4E75        ; $002796
        dc.w    $0239        ; $002798
        dc.w    $0040        ; $00279A
        dc.w    $00A1        ; $00279C
        dc.w    $5181        ; $00279E

; --- Frame buffer write ---
FrameBufferWrite:
        LEA $00840000,A1        ; $0027A0
        MOVE.W #$1F00,D2        ; $0027A6
        MOVE.W #$00DF,D7        ; $0027AA
        MOVE.W D2,(A1)+        ; $0027AE
        DBRA D7,$008827AE        ; $0027B0
        RTS        ; $0027B4
        dc.w    $0239        ; $0027B6
        dc.w    $0040        ; $0027B8
        dc.w    $00A1        ; $0027BA
        dc.w    $5181        ; $0027BC
        dc.w    $43F9        ; $0027BE
        dc.w    $0084        ; $0027C0
        dc.w    $0000        ; $0027C2
        dc.w    $303C        ; $0027C4
        dc.w    $0100        ; $0027C6
        dc.w    $323C        ; $0027C8
        dc.w    $2000        ; $0027CA
        dc.w    $3E3C        ; $0027CC
        dc.w    $00DF        ; $0027CE
        dc.w    $32C1        ; $0027D0
        dc.w    $D240        ; $0027D2
        dc.w    $51CF        ; $0027D4
        dc.w    $FFFA        ; $0027D6
        dc.w    $4E75        ; $0027D8
        dc.w    $6142        ; $0027DA
        dc.w    $49F9        ; $0027DC
        dc.w    $00A1        ; $0027DE
        dc.w    $5100        ; $0027E0
        dc.w    $45F9        ; $0027E2
        dc.w    $00A1        ; $0027E4
        dc.w    $5186        ; $0027E6
        dc.w    $47F9        ; $0027E8
        dc.w    $00A1        ; $0027EA
        dc.w    $5188        ; $0027EC
        dc.w    $323C        ; $0027EE
        dc.w    $2000        ; $0027F0
        dc.w    $6104        ; $0027F2
        dc.w    $323C        ; $0027F4
        dc.w    $F000        ; $0027F6

; --- VDP fill operation ---
VDPFill:
        MOVE.W #$000F,D7        ; $0027F8
        MOVE.W #$0101,D0        ; $0027FC
        MOVE.W #$0100,D2        ; $002800
        MOVE.W #$00FF,$0084(A4)        ; $002804
        MOVE.W D1,(A2)        ; $00280A
        MOVE.W D0,(A3)        ; $00280C
        BTST #1,$008B(A4)        ; $00280E
        BNE $0088280E        ; $002814
        ADD.W D2,D1        ; $002816
        DBRA D7,$0088280A        ; $002818
        RTS        ; $00281C

; --- VDP preparation ---
VDPPrep:
        LEA MARS_SYS_BASE,A4        ; $00281E
        LEA MARS_VDP_FILLADR,A2        ; $002824
        LEA MARS_VDP_FILLDATA,A3        ; $00282A
        MOVE.W #$1F00,D1        ; $002830
        MOVE.W #$0101,D0        ; $002834
        MOVE.W #$00FF,$0084(A4)        ; $002838
        MOVE.W D1,(A2)        ; $00283E
        MOVE.W D0,(A3)        ; $002840
        BTST #1,$008B(A4)        ; $002842
        BNE $00882842        ; $002848
        RTS        ; $00284A

; --- VDP operation 4 ---
VDPOp4:
        LEA $00A15200,A3        ; $00284C
        MOVEQ #$1F,D7        ; $002852
        MOVE.L (A2)+,(A3)+        ; $002854
        MOVE.L (A2)+,(A3)+        ; $002856
        MOVE.L (A2)+,(A3)+        ; $002858
        MOVE.L (A2)+,(A3)+        ; $00285A
        DBRA D7,$00882854        ; $00285C
        RTS        ; $002860

; --- Memory operation 3 ---
MemoryOp3:
        LEA $00A15240,A3        ; $002862
        MOVEQ #$07,D7        ; $002868
        MOVE.L (A2)+,(A3)+        ; $00286A
        MOVE.L (A2)+,(A3)+        ; $00286C
        MOVE.L (A2)+,(A3)+        ; $00286E
        MOVE.L (A2)+,(A3)+        ; $002870
        DBRA D7,$0088286A        ; $002872
        RTS        ; $002876

; --- Palette RAM copy (V-INT state 6) ---
PaletteRAMCopy:
        TST.B $C821.W        ; $002878
        BEQ $0088288E        ; $00287C
        LEA $00FF6E00,A1        ; $00287E
        LEA $00A15200,A2        ; $002884
        DC.W $4EFA ; Unknown        ; $00288A
        MOVEA.L D6,A0        ; $00288C
        RTS        ; $00288E
        dc.w    $0839        ; $002890
        dc.w    $0000        ; $002892
        dc.w    $00A1        ; $002894
        dc.w    $5123        ; $002896
        dc.w    $67F6        ; $002898
        dc.w    $08B9        ; $00289A
        dc.w    $0000        ; $00289C
        dc.w    $00A1        ; $00289E
        dc.w    $5123        ; $0028A0
        dc.w    $31FC        ; $0028A2
        dc.w    $0000        ; $0028A4
        dc.w    $C8A8        ; $0028A6
        dc.w    $13F8        ; $0028A8
        dc.w    $C8A9        ; $0028AA
        dc.w    $00A1        ; $0028AC
        dc.w    $5121        ; $0028AE
        dc.w    $13F8        ; $0028B0
        dc.w    $C8A8        ; $0028B2
        dc.w    $00A1        ; $0028B4
        dc.w    $5120        ; $0028B6
        dc.w    $13FC        ; $0028B8
        dc.w    $0000        ; $0028BA
        dc.w    $00A1        ; $0028BC
        dc.w    $5123        ; $0028BE
        dc.w    $4E75        ; $0028C0

; --- VDP/SH2 COMM synchronization ---
VDPSyncSH2:
        MOVE.W #$0500,MARS_DREQ_LEN        ; $0028C2
        MOVE.B #$0004,MARS_DREQ_CTRL+1        ; $0028CA
        MOVE.B $C8A9.W,$00A15121        ; $0028D2
        MOVE.B $C8A8.W,COMM0        ; $0028DA
        BTST #1,$00A15123        ; $0028E2
        BEQ $008828E2        ; $0028EA
        BCLR #1,$00A15123        ; $0028EC
        LEA $00FF6000,A1        ; $0028F4
        LEA MARS_FIFO,A2        ; $0028FA
        JSR $008988EC        ; $002900
        JSR $008988EC        ; $002906
        JSR $008988EC        ; $00290C
        JSR $008988EC        ; $002912
        JSR $008988EC        ; $002918
        JSR $008988EC        ; $00291E
        JSR $008988EC        ; $002924
        JSR $008988EC        ; $00292A
        JSR $008988EC        ; $002930
        JMP $008988EC        ; $002936
        LEA MARS_SYS_BASE,A4        ; $00293C
        MOVE.B #$0000,(A4)        ; $002942
        LEA MARS_VDP_FILLADR,A2        ; $002946
        LEA MARS_VDP_FILLDATA,A3        ; $00294C
        MOVE.W #$00BF,D7        ; $002952
        MOVEQ #$00,D0        ; $002956
        MOVE.W #$3000,D1        ; $002958
        MOVE.W #$0100,D2        ; $00295C
        MOVE.W #$009F,$0084(A4)        ; $002960
        MOVE.W D1,(A2)        ; $002966
        MOVE.W D0,(A3)        ; $002968
        MOVEQ #$6F,D0        ; $00296A
        OR D0,#$0378        ; $00296C
        BTST #1,$008B(A4)        ; $002970
        BNE $00882970        ; $002976
        ADD.W D2,D1        ; $002978
        DBRA D7,$00882966        ; $00297A
        MOVE.B #$0080,(A4)        ; $00297E
        RTS        ; $002982
        dc.w    $41F8        ; $002984
        dc.w    $9000        ; $002986
        dc.w    $43F9        ; $002988
        dc.w    $00FF        ; $00298A
        dc.w    $6100        ; $00298C
        dc.w    $4A78        ; $00298E
        dc.w    $C04C        ; $002990
        dc.w    $6600        ; $002992
        dc.w    $014A        ; $002994
        dc.w    $3028        ; $002996
        dc.w    $0030        ; $002998
        dc.w    $3340        ; $00299A
        dc.w    $0016        ; $00299C
        dc.w    $3028        ; $00299E
        dc.w    $009C        ; $0029A0
        dc.w    $E940        ; $0029A2
        dc.w    $3200        ; $0029A4
        dc.w    $9278        ; $0029A6
        dc.w    $C054        ; $0029A8
        dc.w    $4441        ; $0029AA
        dc.w    $3341        ; $0029AC
        dc.w    $0004        ; $0029AE
        dc.w    $D078        ; $0029B0
        dc.w    $C0E4        ; $0029B2
        dc.w    $D068        ; $0029B4
        dc.w    $0032        ; $0029B6
        dc.w    $3340        ; $0029B8
        dc.w    $0018        ; $0029BA
        dc.w    $3378        ; $0029BC
        dc.w    $C056        ; $0029BE
        dc.w    $0006        ; $0029C0
        dc.w    $3028        ; $0029C2
        dc.w    $0034        ; $0029C4
        dc.w    $3340        ; $0029C6
        dc.w    $001A        ; $0029C8
        dc.w    $3028        ; $0029CA
        dc.w    $003A        ; $0029CC
        dc.w    $E640        ; $0029CE
        dc.w    $3200        ; $0029D0
        dc.w    $D078        ; $0029D2
        dc.w    $C0C8        ; $0029D4
        dc.w    $3340        ; $0029D6
        dc.w    $0008        ; $0029D8
        dc.w    $3028        ; $0029DA
        dc.w    $0044        ; $0029DC
        dc.w    $E640        ; $0029DE
        dc.w    $D041        ; $0029E0
        dc.w    $4440        ; $0029E2
        dc.w    $3340        ; $0029E4
        dc.w    $001C        ; $0029E6
        dc.w    $3028        ; $0029E8
        dc.w    $003C        ; $0029EA
        dc.w    $D068        ; $0029EC
        dc.w    $0096        ; $0029EE
        dc.w    $E640        ; $0029F0
        dc.w    $3200        ; $0029F2
        dc.w    $D078        ; $0029F4
        dc.w    $C0CA        ; $0029F6
        dc.w    $4440        ; $0029F8
        dc.w    $3340        ; $0029FA
        dc.w    $000A        ; $0029FC
        dc.w    $3028        ; $0029FE
        dc.w    $0046        ; $002A00
        dc.w    $E640        ; $002A02
        dc.w    $9240        ; $002A04
        dc.w    $3341        ; $002A06
        dc.w    $001E        ; $002A08
        dc.w    $3028        ; $002A0A
        dc.w    $003E        ; $002A0C
        dc.w    $E640        ; $002A0E
        dc.w    $3200        ; $002A10
        dc.w    $D078        ; $002A12
        dc.w    $C0CC        ; $002A14
        dc.w    $3340        ; $002A16
        dc.w    $000C        ; $002A18
        dc.w    $3028        ; $002A1A
        dc.w    $004A        ; $002A1C
        dc.w    $D068        ; $002A1E
        dc.w    $004C        ; $002A20
        dc.w    $EA40        ; $002A22
        dc.w    $9041        ; $002A24
        dc.w    $3340        ; $002A26
        dc.w    $0020        ; $002A28
        dc.w    $3028        ; $002A2A
        dc.w    $0090        ; $002A2C
        dc.w    $E640        ; $002A2E
        dc.w    $3340        ; $002A30
        dc.w    $0032        ; $002A32
        dc.w    $3340        ; $002A34
        dc.w    $0046        ; $002A36
        dc.w    $3028        ; $002A38
        dc.w    $00BC        ; $002A3A
        dc.w    $E640        ; $002A3C
        dc.w    $3340        ; $002A3E
        dc.w    $0030        ; $002A40
        dc.w    $3340        ; $002A42
        dc.w    $0044        ; $002A44
        dc.w    $3340        ; $002A46
        dc.w    $0058        ; $002A48
        dc.w    $4A38        ; $002A4A
        dc.w    $C31C        ; $002A4C
        dc.w    $6722        ; $002A4E
        dc.w    $0828        ; $002A50
        dc.w    $0003        ; $002A52
        dc.w    $00E5        ; $002A54
        dc.w    $671A        ; $002A56
        dc.w    $7000        ; $002A58
        dc.w    $3280        ; $002A5A
        dc.w    $3340        ; $002A5C
        dc.w    $0014        ; $002A5E
        dc.w    $3340        ; $002A60
        dc.w    $0028        ; $002A62
        dc.w    $3340        ; $002A64
        dc.w    $003C        ; $002A66
        dc.w    $3340        ; $002A68
        dc.w    $0050        ; $002A6A
        dc.w    $3340        ; $002A6C
        dc.w    $0064        ; $002A6E
        dc.w    $4E75        ; $002A70
        dc.w    $7001        ; $002A72
        dc.w    $3280        ; $002A74
        dc.w    $3340        ; $002A76
        dc.w    $0014        ; $002A78
        dc.w    $3340        ; $002A7A
        dc.w    $0028        ; $002A7C
        dc.w    $3340        ; $002A7E
        dc.w    $003C        ; $002A80
        dc.w    $3340        ; $002A82
        dc.w    $0050        ; $002A84
        dc.w    $3028        ; $002A86
        dc.w    $008A        ; $002A88
        dc.w    $6738        ; $002A8A
        dc.w    $5340        ; $002A8C
        dc.w    $671A        ; $002A8E
        dc.w    $2378        ; $002A90
        dc.w    $C74C        ; $002A92
        dc.w    $0024        ; $002A94
        dc.w    $337C        ; $002A96
        dc.w    $0001        ; $002A98
        dc.w    $0064        ; $002A9A
        dc.w    $4A68        ; $002A9C
        dc.w    $008C        ; $002A9E
        dc.w    $673A        ; $002AA0
        dc.w    $337C        ; $002AA2
        dc.w    $0000        ; $002AA4
        dc.w    $0064        ; $002AA6
        dc.w    $4E75        ; $002AA8
        dc.w    $2378        ; $002AAA
        dc.w    $C748        ; $002AAC
        dc.w    $0024        ; $002AAE
        dc.w    $337C        ; $002AB0
        dc.w    $0001        ; $002AB2
        dc.w    $0064        ; $002AB4
        dc.w    $4A68        ; $002AB6
        dc.w    $008C        ; $002AB8
        dc.w    $6720        ; $002ABA
        dc.w    $337C        ; $002ABC
        dc.w    $0000        ; $002ABE
        dc.w    $0064        ; $002AC0
        dc.w    $4E75        ; $002AC2
        dc.w    $2378        ; $002AC4
        dc.w    $C710        ; $002AC6
        dc.w    $0024        ; $002AC8
        dc.w    $337C        ; $002ACA
        dc.w    $0001        ; $002ACC
        dc.w    $0064        ; $002ACE
        dc.w    $4A68        ; $002AD0
        dc.w    $008C        ; $002AD2
        dc.w    $6706        ; $002AD4
        dc.w    $337C        ; $002AD6
        dc.w    $0000        ; $002AD8
        dc.w    $0064        ; $002ADA
        dc.w    $4E75        ; $002ADC
        dc.w    $2378        ; $002ADE
        dc.w    $C724        ; $002AE0
        dc.w    $0024        ; $002AE2
        dc.w    $4A68        ; $002AE4
        dc.w    $008A        ; $002AE6
        dc.w    $6706        ; $002AE8
        dc.w    $2378        ; $002AEA
        dc.w    $C750        ; $002AEC
        dc.w    $0024        ; $002AEE
        dc.w    $3028        ; $002AF0
        dc.w    $0030        ; $002AF2
        dc.w    $3340        ; $002AF4
        dc.w    $0016        ; $002AF6
        dc.w    $3378        ; $002AF8
        dc.w    $C054        ; $002AFA
        dc.w    $0004        ; $002AFC
        dc.w    $3028        ; $002AFE
        dc.w    $009C        ; $002B00
        dc.w    $E940        ; $002B02
        dc.w    $D068        ; $002B04
        dc.w    $0032        ; $002B06
        dc.w    $D078        ; $002B08
        dc.w    $C0E4        ; $002B0A
        dc.w    $3340        ; $002B0C
        dc.w    $0018        ; $002B0E
        dc.w    $3378        ; $002B10
        dc.w    $C056        ; $002B12
        dc.w    $0006        ; $002B14
        dc.w    $3028        ; $002B16
        dc.w    $0034        ; $002B18
        dc.w    $3340        ; $002B1A
        dc.w    $001A        ; $002B1C
        dc.w    $3028        ; $002B1E
        dc.w    $003A        ; $002B20
        dc.w    $E640        ; $002B22
        dc.w    $3200        ; $002B24
        dc.w    $D078        ; $002B26
        dc.w    $C0C8        ; $002B28
        dc.w    $3340        ; $002B2A
        dc.w    $0008        ; $002B2C
        dc.w    $4441        ; $002B2E
        dc.w    $3341        ; $002B30
        dc.w    $001C        ; $002B32
        dc.w    $3028        ; $002B34
        dc.w    $003C        ; $002B36
        dc.w    $D068        ; $002B38
        dc.w    $0096        ; $002B3A
        dc.w    $9068        ; $002B3C
        dc.w    $0046        ; $002B3E
        dc.w    $E640        ; $002B40
        dc.w    $3200        ; $002B42
        dc.w    $D078        ; $002B44
        dc.w    $C0CA        ; $002B46
        dc.w    $4440        ; $002B48
        dc.w    $3340        ; $002B4A
        dc.w    $000A        ; $002B4C
        dc.w    $3341        ; $002B4E
        dc.w    $001E        ; $002B50
        dc.w    $3028        ; $002B52
        dc.w    $003E        ; $002B54
        dc.w    $E640        ; $002B56
        dc.w    $3200        ; $002B58
        dc.w    $D078        ; $002B5A
        dc.w    $C0CC        ; $002B5C
        dc.w    $3340        ; $002B5E
        dc.w    $000C        ; $002B60
        dc.w    $3028        ; $002B62
        dc.w    $004C        ; $002B64
        dc.w    $E840        ; $002B66
        dc.w    $9041        ; $002B68
        dc.w    $3340        ; $002B6A
        dc.w    $0020        ; $002B6C
        dc.w    $3028        ; $002B6E
        dc.w    $0090        ; $002B70
        dc.w    $E640        ; $002B72
        dc.w    $3340        ; $002B74
        dc.w    $0032        ; $002B76
        dc.w    $3340        ; $002B78
        dc.w    $0046        ; $002B7A
        dc.w    $3028        ; $002B7C
        dc.w    $00BC        ; $002B7E
        dc.w    $E640        ; $002B80
        dc.w    $3340        ; $002B82
        dc.w    $0030        ; $002B84
        dc.w    $3340        ; $002B86
        dc.w    $0044        ; $002B88
        dc.w    $3238        ; $002B8A
        dc.w    $C088        ; $002B8C
        dc.w    $3028        ; $002B8E
        dc.w    $008E        ; $002B90
        dc.w    $48C1        ; $002B92
        dc.w    $48C0        ; $002B94
        dc.w    $D081        ; $002B96
        dc.w    $E280        ; $002B98
        dc.w    $31C0        ; $002B9A
        dc.w    $C088        ; $002B9C
        dc.w    $4440        ; $002B9E
        dc.w    $3200        ; $002BA0
        dc.w    $EE41        ; $002BA2
        dc.w    $EC40        ; $002BA4
        dc.w    $D041        ; $002BA6
        dc.w    $E340        ; $002BA8
        dc.w    $3340        ; $002BAA
        dc.w    $0070        ; $002BAC
        dc.w    $4E75        ; $002BAE
        dc.w    $41F8        ; $002BB0
        dc.w    $9000        ; $002BB2
        dc.w    $43F9        ; $002BB4
        dc.w    $00FF        ; $002BB6
        dc.w    $6100        ; $002BB8
        dc.w    $45F9        ; $002BBA
        dc.w    $00FF        ; $002BBC
        dc.w    $6330        ; $002BBE
        dc.w    $0838        ; $002BC0
        dc.w    $0005        ; $002BC2
        dc.w    $C30E        ; $002BC4
        dc.w    $661E        ; $002BC6
        dc.w    $4EBA        ; $002BC8
        dc.w    $008E        ; $002BCA
        dc.w    $4A78        ; $002BCC
        dc.w    $C04C        ; $002BCE
        dc.w    $660C        ; $002BD0
        dc.w    $4EBA        ; $002BD2
        dc.w    $0108        ; $002BD4
        dc.w    $4EBA        ; $002BD6
        dc.w    $01F2        ; $002BD8
        dc.w    $4EFA        ; $002BDA
        dc.w    $02EA        ; $002BDC
        dc.w    $4EBA        ; $002BDE
        dc.w    $0324        ; $002BE0
        dc.w    $4EFA        ; $002BE2
        dc.w    $02BA        ; $002BE4
        dc.w    $7002        ; $002BE6
        dc.w    $3340        ; $002BE8
        dc.w    $0014        ; $002BEA
        dc.w    $3340        ; $002BEC
        dc.w    $0028        ; $002BEE
        dc.w    $3340        ; $002BF0
        dc.w    $003C        ; $002BF2
        dc.w    $3340        ; $002BF4
        dc.w    $0050        ; $002BF6
        dc.w    $4EBA        ; $002BF8
        dc.w    $0536        ; $002BFA
        dc.w    $4EBA        ; $002BFC
        dc.w    $0412        ; $002BFE
        dc.w    $4EFA        ; $002C00
        dc.w    $01C8        ; $002C02
        dc.w    $41F8        ; $002C04
        dc.w    $9F00        ; $002C06
        dc.w    $43F9        ; $002C08
        dc.w    $00FF        ; $002C0A
        dc.w    $6330        ; $002C0C
        dc.w    $45F9        ; $002C0E
        dc.w    $00FF        ; $002C10
        dc.w    $6100        ; $002C12
        dc.w    $0838        ; $002C14
        dc.w    $0005        ; $002C16
        dc.w    $C30E        ; $002C18
        dc.w    $661E        ; $002C1A
        dc.w    $4EBA        ; $002C1C
        dc.w    $007C        ; $002C1E
        dc.w    $4A78        ; $002C20
        dc.w    $C04C        ; $002C22
        dc.w    $660C        ; $002C24
        dc.w    $4EBA        ; $002C26
        dc.w    $00B4        ; $002C28
        dc.w    $4EBA        ; $002C2A
        dc.w    $0208        ; $002C2C
        dc.w    $4EFA        ; $002C2E
        dc.w    $0296        ; $002C30
        dc.w    $4EBA        ; $002C32
        dc.w    $02D0        ; $002C34
        dc.w    $4EFA        ; $002C36
        dc.w    $027A        ; $002C38
        dc.w    $7002        ; $002C3A
        dc.w    $3340        ; $002C3C
        dc.w    $0014        ; $002C3E
        dc.w    $3340        ; $002C40
        dc.w    $0028        ; $002C42
        dc.w    $3340        ; $002C44
        dc.w    $003C        ; $002C46
        dc.w    $3340        ; $002C48
        dc.w    $0050        ; $002C4A
        dc.w    $4EBA        ; $002C4C
        dc.w    $04E2        ; $002C4E
        dc.w    $4EBA        ; $002C50
        dc.w    $03BE        ; $002C52
        dc.w    $4EFA        ; $002C54
        dc.w    $01DE        ; $002C56
        dc.w    $7000        ; $002C58
        dc.w    $4A68        ; $002C5A
        dc.w    $00C0        ; $002C5C
        dc.w    $6728        ; $002C5E
        dc.w    $7001        ; $002C60
        dc.w    $4A38        ; $002C62
        dc.w    $9FE4        ; $002C64
        dc.w    $6606        ; $002C66
        dc.w    $4A38        ; $002C68
        dc.w    $B4FC        ; $002C6A
        dc.w    $670C        ; $002C6C
        dc.w    $0828        ; $002C6E
        dc.w    $0003        ; $002C70
        dc.w    $00E5        ; $002C72
        dc.w    $6712        ; $002C74
        dc.w    $7000        ; $002C76
        dc.w    $600E        ; $002C78
        dc.w    $0838        ; $002C7A
        dc.w    $0003        ; $002C7C
        dc.w    $9FE5        ; $002C7E
        dc.w    $6706        ; $002C80
        dc.w    $4A28        ; $002C82
        dc.w    $00E4        ; $002C84
        dc.w    $66EE        ; $002C86
        dc.w    $3540        ; $002C88
        dc.w    $0118        ; $002C8A
        dc.w    $3540        ; $002C8C
        dc.w    $012C        ; $002C8E
        dc.w    $3540        ; $002C90
        dc.w    $0140        ; $002C92
        dc.w    $3540        ; $002C94
        dc.w    $0154        ; $002C96
        dc.w    $4E75        ; $002C98
        dc.w    $7000        ; $002C9A
        dc.w    $4A68        ; $002C9C
        dc.w    $00C0        ; $002C9E
        dc.w    $6728        ; $002CA0
        dc.w    $7001        ; $002CA2
        dc.w    $4A38        ; $002CA4
        dc.w    $90E4        ; $002CA6
        dc.w    $6606        ; $002CA8
        dc.w    $4A38        ; $002CAA
        dc.w    $B0FC        ; $002CAC
        dc.w    $670C        ; $002CAE
        dc.w    $0828        ; $002CB0
        dc.w    $0003        ; $002CB2
        dc.w    $00E5        ; $002CB4
        dc.w    $6712        ; $002CB6
        dc.w    $7000        ; $002CB8
        dc.w    $600E        ; $002CBA
        dc.w    $0838        ; $002CBC
        dc.w    $0003        ; $002CBE
        dc.w    $90E5        ; $002CC0
        dc.w    $6706        ; $002CC2
        dc.w    $4A28        ; $002CC4
        dc.w    $00E4        ; $002CC6
        dc.w    $66EE        ; $002CC8
        dc.w    $3540        ; $002CCA
        dc.w    $0118        ; $002CCC
        dc.w    $3540        ; $002CCE
        dc.w    $012C        ; $002CD0
        dc.w    $3540        ; $002CD2
        dc.w    $0140        ; $002CD4
        dc.w    $3540        ; $002CD6
        dc.w    $0154        ; $002CD8
        dc.w    $4E75        ; $002CDA
        dc.w    $3028        ; $002CDC
        dc.w    $0030        ; $002CDE
        dc.w    $3340        ; $002CE0
        dc.w    $0016        ; $002CE2
        dc.w    $3540        ; $002CE4
        dc.w    $011A        ; $002CE6
        dc.w    $3028        ; $002CE8
        dc.w    $009C        ; $002CEA
        dc.w    $E940        ; $002CEC
        dc.w    $3200        ; $002CEE
        dc.w    $9278        ; $002CF0
        dc.w    $C054        ; $002CF2
        dc.w    $4441        ; $002CF4
        dc.w    $3341        ; $002CF6
        dc.w    $0004        ; $002CF8
        dc.w    $D078        ; $002CFA
        dc.w    $C0E4        ; $002CFC
        dc.w    $D068        ; $002CFE
        dc.w    $0032        ; $002D00
        dc.w    $3340        ; $002D02
        dc.w    $0018        ; $002D04
        dc.w    $3540        ; $002D06
        dc.w    $011C        ; $002D08
        dc.w    $3378        ; $002D0A
        dc.w    $C056        ; $002D0C
        dc.w    $0006        ; $002D0E
        dc.w    $3028        ; $002D10
        dc.w    $0034        ; $002D12
        dc.w    $3340        ; $002D14
        dc.w    $001A        ; $002D16
        dc.w    $3540        ; $002D18
        dc.w    $011E        ; $002D1A
        dc.w    $3028        ; $002D1C
        dc.w    $003A        ; $002D1E
        dc.w    $E640        ; $002D20
        dc.w    $3200        ; $002D22
        dc.w    $D078        ; $002D24
        dc.w    $C0C8        ; $002D26
        dc.w    $D078        ; $002D28
        dc.w    $C0AE        ; $002D2A
        dc.w    $3340        ; $002D2C
        dc.w    $0008        ; $002D2E
        dc.w    $3028        ; $002D30
        dc.w    $0044        ; $002D32
        dc.w    $E640        ; $002D34
        dc.w    $D041        ; $002D36
        dc.w    $4440        ; $002D38
        dc.w    $3340        ; $002D3A
        dc.w    $001C        ; $002D3C
        dc.w    $3540        ; $002D3E
        dc.w    $0120        ; $002D40
        dc.w    $3028        ; $002D42
        dc.w    $003C        ; $002D44
        dc.w    $D068        ; $002D46
        dc.w    $0096        ; $002D48
        dc.w    $E640        ; $002D4A
        dc.w    $3200        ; $002D4C
        dc.w    $D078        ; $002D4E
        dc.w    $C0CA        ; $002D50
        dc.w    $D078        ; $002D52
        dc.w    $C0B0        ; $002D54
        dc.w    $4440        ; $002D56
        dc.w    $3340        ; $002D58
        dc.w    $000A        ; $002D5A
        dc.w    $3028        ; $002D5C
        dc.w    $0046        ; $002D5E
        dc.w    $E640        ; $002D60
        dc.w    $9240        ; $002D62
        dc.w    $3341        ; $002D64
        dc.w    $001E        ; $002D66
        dc.w    $3541        ; $002D68
        dc.w    $0122        ; $002D6A
        dc.w    $3028        ; $002D6C
        dc.w    $003E        ; $002D6E
        dc.w    $E640        ; $002D70
        dc.w    $3200        ; $002D72
        dc.w    $D078        ; $002D74
        dc.w    $C0CC        ; $002D76
        dc.w    $D078        ; $002D78
        dc.w    $C0B2        ; $002D7A
        dc.w    $3340        ; $002D7C
        dc.w    $000C        ; $002D7E
        dc.w    $3028        ; $002D80
        dc.w    $004A        ; $002D82
        dc.w    $D068        ; $002D84
        dc.w    $004C        ; $002D86
        dc.w    $EA40        ; $002D88
        dc.w    $9041        ; $002D8A
        dc.w    $3340        ; $002D8C
        dc.w    $0020        ; $002D8E
        dc.w    $3540        ; $002D90
        dc.w    $0124        ; $002D92
        dc.w    $3028        ; $002D94
        dc.w    $0090        ; $002D96
        dc.w    $E640        ; $002D98
        dc.w    $3340        ; $002D9A
        dc.w    $0032        ; $002D9C
        dc.w    $3340        ; $002D9E
        dc.w    $0046        ; $002DA0
        dc.w    $3540        ; $002DA2
        dc.w    $0136        ; $002DA4
        dc.w    $3540        ; $002DA6
        dc.w    $014A        ; $002DA8
        dc.w    $3028        ; $002DAA
        dc.w    $00BC        ; $002DAC
        dc.w    $E640        ; $002DAE
        dc.w    $3340        ; $002DB0
        dc.w    $0030        ; $002DB2
        dc.w    $3340        ; $002DB4
        dc.w    $0044        ; $002DB6
        dc.w    $3340        ; $002DB8
        dc.w    $0058        ; $002DBA
        dc.w    $3540        ; $002DBC
        dc.w    $0134        ; $002DBE
        dc.w    $3540        ; $002DC0
        dc.w    $0148        ; $002DC2
        dc.w    $3540        ; $002DC4
        dc.w    $015C        ; $002DC6
        dc.w    $4E75        ; $002DC8
        dc.w    $3028        ; $002DCA
        dc.w    $008A        ; $002DCC
        dc.w    $6744        ; $002DCE
        dc.w    $5340        ; $002DD0
        dc.w    $6720        ; $002DD2
        dc.w    $2378        ; $002DD4
        dc.w    $C74C        ; $002DD6
        dc.w    $0024        ; $002DD8
        dc.w    $2578        ; $002DDA
        dc.w    $C74C        ; $002DDC
        dc.w    $0128        ; $002DDE
        dc.w    $337C        ; $002DE0
        dc.w    $0001        ; $002DE2
        dc.w    $0064        ; $002DE4
        dc.w    $4A68        ; $002DE6
        dc.w    $008C        ; $002DE8
        dc.w    $6746        ; $002DEA
        dc.w    $337C        ; $002DEC
        dc.w    $0000        ; $002DEE
        dc.w    $0064        ; $002DF0
        dc.w    $4E75        ; $002DF2
        dc.w    $2378        ; $002DF4
        dc.w    $C748        ; $002DF6
        dc.w    $0024        ; $002DF8
        dc.w    $2578        ; $002DFA
        dc.w    $C748        ; $002DFC
        dc.w    $0128        ; $002DFE
        dc.w    $337C        ; $002E00
        dc.w    $0001        ; $002E02
        dc.w    $0064        ; $002E04
        dc.w    $4A68        ; $002E06
        dc.w    $008C        ; $002E08
        dc.w    $6726        ; $002E0A
        dc.w    $337C        ; $002E0C
        dc.w    $0000        ; $002E0E
        dc.w    $0064        ; $002E10
        dc.w    $4E75        ; $002E12
        dc.w    $2378        ; $002E14
        dc.w    $C710        ; $002E16
        dc.w    $0024        ; $002E18
        dc.w    $2578        ; $002E1A
        dc.w    $C710        ; $002E1C
        dc.w    $0128        ; $002E1E
        dc.w    $337C        ; $002E20
        dc.w    $0001        ; $002E22
        dc.w    $0064        ; $002E24
        dc.w    $4A68        ; $002E26
        dc.w    $008C        ; $002E28
        dc.w    $6706        ; $002E2A
        dc.w    $337C        ; $002E2C
        dc.w    $0000        ; $002E2E
        dc.w    $0064        ; $002E30
        dc.w    $4E75        ; $002E32
        dc.w    $3028        ; $002E34
        dc.w    $008A        ; $002E36
        dc.w    $6744        ; $002E38
        dc.w    $5340        ; $002E3A
        dc.w    $6720        ; $002E3C
        dc.w    $2378        ; $002E3E
        dc.w    $C760        ; $002E40
        dc.w    $0024        ; $002E42
        dc.w    $2578        ; $002E44
        dc.w    $C760        ; $002E46
        dc.w    $0128        ; $002E48
        dc.w    $337C        ; $002E4A
        dc.w    $0001        ; $002E4C
        dc.w    $0064        ; $002E4E
        dc.w    $4A68        ; $002E50
        dc.w    $008C        ; $002E52
        dc.w    $6746        ; $002E54
        dc.w    $337C        ; $002E56
        dc.w    $0000        ; $002E58
        dc.w    $0064        ; $002E5A
        dc.w    $4E75        ; $002E5C
        dc.w    $2378        ; $002E5E
        dc.w    $C75C        ; $002E60
        dc.w    $0024        ; $002E62
        dc.w    $2578        ; $002E64
        dc.w    $C75C        ; $002E66
        dc.w    $0128        ; $002E68
        dc.w    $337C        ; $002E6A
        dc.w    $0001        ; $002E6C
        dc.w    $0064        ; $002E6E
        dc.w    $4A68        ; $002E70
        dc.w    $008C        ; $002E72
        dc.w    $6726        ; $002E74
        dc.w    $337C        ; $002E76
        dc.w    $0000        ; $002E78
        dc.w    $0064        ; $002E7A
        dc.w    $4E75        ; $002E7C
        dc.w    $2378        ; $002E7E
        dc.w    $C754        ; $002E80
        dc.w    $0024        ; $002E82
        dc.w    $2578        ; $002E84
        dc.w    $C754        ; $002E86
        dc.w    $0128        ; $002E88
        dc.w    $337C        ; $002E8A
        dc.w    $0001        ; $002E8C
        dc.w    $0064        ; $002E8E
        dc.w    $4A68        ; $002E90
        dc.w    $008C        ; $002E92
        dc.w    $6706        ; $002E94
        dc.w    $337C        ; $002E96
        dc.w    $0000        ; $002E98
        dc.w    $0064        ; $002E9A
        dc.w    $4E75        ; $002E9C
        dc.w    $2378        ; $002E9E
        dc.w    $C724        ; $002EA0
        dc.w    $0024        ; $002EA2
        dc.w    $4A68        ; $002EA4
        dc.w    $008A        ; $002EA6
        dc.w    $6706        ; $002EA8
        dc.w    $2378        ; $002EAA
        dc.w    $C750        ; $002EAC
        dc.w    $0024        ; $002EAE
        dc.w    $4E75        ; $002EB0
        dc.w    $2378        ; $002EB2
        dc.w    $C758        ; $002EB4
        dc.w    $0024        ; $002EB6
        dc.w    $4A68        ; $002EB8
        dc.w    $008A        ; $002EBA
        dc.w    $6706        ; $002EBC
        dc.w    $2378        ; $002EBE
        dc.w    $C764        ; $002EC0
        dc.w    $0024        ; $002EC2
        dc.w    $4E75        ; $002EC4
        dc.w    $4A38        ; $002EC6
        dc.w    $C31C        ; $002EC8
        dc.w    $6722        ; $002ECA
        dc.w    $0828        ; $002ECC
        dc.w    $0003        ; $002ECE
        dc.w    $00E5        ; $002ED0
        dc.w    $671A        ; $002ED2
        dc.w    $7000        ; $002ED4
        dc.w    $3280        ; $002ED6
        dc.w    $3340        ; $002ED8
        dc.w    $0014        ; $002EDA
        dc.w    $3340        ; $002EDC
        dc.w    $0028        ; $002EDE
        dc.w    $3340        ; $002EE0
        dc.w    $003C        ; $002EE2
        dc.w    $3340        ; $002EE4
        dc.w    $0050        ; $002EE6
        dc.w    $3340        ; $002EE8
        dc.w    $0064        ; $002EEA
        dc.w    $4E75        ; $002EEC
        dc.w    $7001        ; $002EEE
        dc.w    $3280        ; $002EF0
        dc.w    $3340        ; $002EF2
        dc.w    $0014        ; $002EF4
        dc.w    $3340        ; $002EF6
        dc.w    $0028        ; $002EF8
        dc.w    $3340        ; $002EFA
        dc.w    $003C        ; $002EFC
        dc.w    $3340        ; $002EFE
        dc.w    $0050        ; $002F00
        dc.w    $4E75        ; $002F02
        dc.w    $3028        ; $002F04
        dc.w    $0030        ; $002F06
        dc.w    $3340        ; $002F08
        dc.w    $0016        ; $002F0A
        dc.w    $3540        ; $002F0C
        dc.w    $011A        ; $002F0E
        dc.w    $3378        ; $002F10
        dc.w    $C054        ; $002F12
        dc.w    $0004        ; $002F14
        dc.w    $3028        ; $002F16
        dc.w    $009C        ; $002F18
        dc.w    $E940        ; $002F1A
        dc.w    $D068        ; $002F1C
        dc.w    $0032        ; $002F1E
        dc.w    $D078        ; $002F20
        dc.w    $C0E4        ; $002F22
        dc.w    $3340        ; $002F24
        dc.w    $0018        ; $002F26
        dc.w    $3540        ; $002F28
        dc.w    $011C        ; $002F2A
        dc.w    $3378        ; $002F2C
        dc.w    $C056        ; $002F2E
        dc.w    $0006        ; $002F30
        dc.w    $3028        ; $002F32
        dc.w    $0034        ; $002F34
        dc.w    $3340        ; $002F36
        dc.w    $001A        ; $002F38
        dc.w    $3540        ; $002F3A
        dc.w    $011E        ; $002F3C
        dc.w    $3028        ; $002F3E
        dc.w    $003A        ; $002F40
        dc.w    $E640        ; $002F42
        dc.w    $3200        ; $002F44
        dc.w    $D078        ; $002F46
        dc.w    $C0C8        ; $002F48
        dc.w    $D078        ; $002F4A
        dc.w    $C0AE        ; $002F4C
        dc.w    $3340        ; $002F4E
        dc.w    $0008        ; $002F50
        dc.w    $4441        ; $002F52
        dc.w    $3341        ; $002F54
        dc.w    $001C        ; $002F56
        dc.w    $3541        ; $002F58
        dc.w    $0120        ; $002F5A
        dc.w    $3028        ; $002F5C
        dc.w    $003C        ; $002F5E
        dc.w    $D068        ; $002F60
        dc.w    $0096        ; $002F62
        dc.w    $9068        ; $002F64
        dc.w    $0046        ; $002F66
        dc.w    $E640        ; $002F68
        dc.w    $3200        ; $002F6A
        dc.w    $D078        ; $002F6C
        dc.w    $C0CA        ; $002F6E
        dc.w    $D078        ; $002F70
        dc.w    $C0B0        ; $002F72
        dc.w    $4440        ; $002F74
        dc.w    $3340        ; $002F76
        dc.w    $000A        ; $002F78
        dc.w    $3341        ; $002F7A
        dc.w    $001E        ; $002F7C
        dc.w    $3028        ; $002F7E
        dc.w    $0046        ; $002F80
        dc.w    $E640        ; $002F82
        dc.w    $9240        ; $002F84
        dc.w    $3541        ; $002F86
        dc.w    $0122        ; $002F88
        dc.w    $3028        ; $002F8A
        dc.w    $003E        ; $002F8C
        dc.w    $E640        ; $002F8E
        dc.w    $3200        ; $002F90
        dc.w    $D078        ; $002F92
        dc.w    $C0CC        ; $002F94
        dc.w    $D078        ; $002F96
        dc.w    $C0B2        ; $002F98
        dc.w    $3340        ; $002F9A
        dc.w    $000C        ; $002F9C
        dc.w    $3028        ; $002F9E
        dc.w    $004C        ; $002FA0
        dc.w    $E840        ; $002FA2
        dc.w    $9041        ; $002FA4
        dc.w    $3340        ; $002FA6
        dc.w    $0020        ; $002FA8
        dc.w    $3028        ; $002FAA
        dc.w    $004A        ; $002FAC
        dc.w    $D068        ; $002FAE
        dc.w    $004C        ; $002FB0
        dc.w    $EA40        ; $002FB2
        dc.w    $9041        ; $002FB4
        dc.w    $3540        ; $002FB6
        dc.w    $0124        ; $002FB8
        dc.w    $3028        ; $002FBA
        dc.w    $0090        ; $002FBC
        dc.w    $E640        ; $002FBE
        dc.w    $3340        ; $002FC0
        dc.w    $0032        ; $002FC2
        dc.w    $3340        ; $002FC4
        dc.w    $0046        ; $002FC6
        dc.w    $3540        ; $002FC8
        dc.w    $0136        ; $002FCA
        dc.w    $3540        ; $002FCC
        dc.w    $014A        ; $002FCE
        dc.w    $3028        ; $002FD0
        dc.w    $00BC        ; $002FD2
        dc.w    $E640        ; $002FD4
        dc.w    $3340        ; $002FD6
        dc.w    $0030        ; $002FD8
        dc.w    $3340        ; $002FDA
        dc.w    $0044        ; $002FDC
        dc.w    $3540        ; $002FDE
        dc.w    $0134        ; $002FE0
        dc.w    $3540        ; $002FE2
        dc.w    $0148        ; $002FE4
        dc.w    $3540        ; $002FE6
        dc.w    $015C        ; $002FE8
        dc.w    $3238        ; $002FEA
        dc.w    $C088        ; $002FEC
        dc.w    $3028        ; $002FEE
        dc.w    $008E        ; $002FF0
        dc.w    $48C1        ; $002FF2
        dc.w    $48C0        ; $002FF4
        dc.w    $D081        ; $002FF6
        dc.w    $E280        ; $002FF8
        dc.w    $31C0        ; $002FFA
        dc.w    $C088        ; $002FFC
        dc.w    $4440        ; $002FFE
        dc.w    $3200        ; $003000
        dc.w    $EE41        ; $003002
        dc.w    $EC40        ; $003004
        dc.w    $D041        ; $003006
        dc.w    $E340        ; $003008
        dc.w    $3340        ; $00300A
        dc.w    $0070        ; $00300C
        dc.w    $4E75        ; $00300E
        dc.w    $7000        ; $003010
        dc.w    $4A68        ; $003012
        dc.w    $00C0        ; $003014
        dc.w    $6702        ; $003016
        dc.w    $7001        ; $003018
        dc.w    $3540        ; $00301A
        dc.w    $0118        ; $00301C
        dc.w    $3540        ; $00301E
        dc.w    $012C        ; $003020
        dc.w    $3540        ; $003022
        dc.w    $0140        ; $003024
        dc.w    $3540        ; $003026
        dc.w    $0154        ; $003028
        dc.w    $3028        ; $00302A
        dc.w    $0030        ; $00302C
        dc.w    $3540        ; $00302E
        dc.w    $011A        ; $003030
        dc.w    $3028        ; $003032
        dc.w    $009C        ; $003034
        dc.w    $E940        ; $003036
        dc.w    $D078        ; $003038
        dc.w    $C0E4        ; $00303A
        dc.w    $D068        ; $00303C
        dc.w    $0032        ; $00303E
        dc.w    $3540        ; $003040
        dc.w    $011C        ; $003042
        dc.w    $3028        ; $003044
        dc.w    $0034        ; $003046
        dc.w    $3540        ; $003048
        dc.w    $011E        ; $00304A
        dc.w    $3228        ; $00304C
        dc.w    $003A        ; $00304E
        dc.w    $E641        ; $003050
        dc.w    $3028        ; $003052
        dc.w    $0044        ; $003054
        dc.w    $E640        ; $003056
        dc.w    $D041        ; $003058
        dc.w    $4440        ; $00305A
        dc.w    $3540        ; $00305C
        dc.w    $0120        ; $00305E
        dc.w    $3428        ; $003060
        dc.w    $003C        ; $003062
        dc.w    $D468        ; $003064
        dc.w    $0096        ; $003066
        dc.w    $E642        ; $003068
        dc.w    $3028        ; $00306A
        dc.w    $0046        ; $00306C
        dc.w    $E640        ; $00306E
        dc.w    $4440        ; $003070
        dc.w    $D042        ; $003072
        dc.w    $3540        ; $003074
        dc.w    $0122        ; $003076
        dc.w    $3628        ; $003078
        dc.w    $003E        ; $00307A
        dc.w    $E643        ; $00307C
        dc.w    $3028        ; $00307E
        dc.w    $004A        ; $003080
        dc.w    $D068        ; $003082
        dc.w    $004C        ; $003084
        dc.w    $EA40        ; $003086
        dc.w    $D043        ; $003088
        dc.w    $4440        ; $00308A
        dc.w    $3540        ; $00308C
        dc.w    $0124        ; $00308E
        dc.w    $3028        ; $003090
        dc.w    $0090        ; $003092
        dc.w    $E640        ; $003094
        dc.w    $3340        ; $003096
        dc.w    $0032        ; $003098
        dc.w    $3340        ; $00309A
        dc.w    $0046        ; $00309C
        dc.w    $3540        ; $00309E
        dc.w    $0136        ; $0030A0
        dc.w    $3540        ; $0030A2
        dc.w    $014A        ; $0030A4
        dc.w    $3028        ; $0030A6
        dc.w    $00BC        ; $0030A8
        dc.w    $E640        ; $0030AA
        dc.w    $3340        ; $0030AC
        dc.w    $0030        ; $0030AE
        dc.w    $3340        ; $0030B0
        dc.w    $0044        ; $0030B2
        dc.w    $3340        ; $0030B4
        dc.w    $0058        ; $0030B6
        dc.w    $3540        ; $0030B8
        dc.w    $0134        ; $0030BA
        dc.w    $3540        ; $0030BC
        dc.w    $0148        ; $0030BE
        dc.w    $3540        ; $0030C0
        dc.w    $015C        ; $0030C2
        dc.w    $4E75        ; $0030C4
        dc.w    $0838        ; $0030C6
        dc.w    $0005        ; $0030C8
        dc.w    $C30E        ; $0030CA
        dc.w    $6746        ; $0030CC
        dc.w    $43F9        ; $0030CE
        dc.w    $00FF        ; $0030D0
        dc.w    $6100        ; $0030D2
        dc.w    $3029        ; $0030D4
        dc.w    $000A        ; $0030D6
        dc.w    $9078        ; $0030D8
        dc.w    $C0B0        ; $0030DA
        dc.w    $323C        ; $0030DC
        dc.w    $F000        ; $0030DE
        dc.w    $B041        ; $0030E0
        dc.w    $6F08        ; $0030E2
        dc.w    $0678        ; $0030E4
        dc.w    $0040        ; $0030E6
        dc.w    $C0B0        ; $0030E8
        dc.w    $3200        ; $0030EA
        dc.w    $3341        ; $0030EC
        dc.w    $000A        ; $0030EE
        dc.w    $3038        ; $0030F0
        dc.w    $C056        ; $0030F2
        dc.w    $323C        ; $0030F4
        dc.w    $0280        ; $0030F6
        dc.w    $4A78        ; $0030F8
        dc.w    $C8C8        ; $0030FA
        dc.w    $6704        ; $0030FC
        dc.w    $323C        ; $0030FE
        dc.w    $0350        ; $003100
        dc.w    $B041        ; $003102
        dc.w    $6F0A        ; $003104
        dc.w    $0478        ; $003106
        dc.w    $0010        ; $003108
        dc.w    $C056        ; $00310A
        dc.w    $3238        ; $00310C
        dc.w    $C056        ; $00310E
        dc.w    $31C1        ; $003110
        dc.w    $C056        ; $003112
        dc.w    $4E75        ; $003114
        dc.w    $0838        ; $003116
        dc.w    $0005        ; $003118
        dc.w    $C30E        ; $00311A
        dc.w    $6706        ; $00311C
        dc.w    $0678        ; $00311E
        dc.w    $0040        ; $003120
        dc.w    $C0B0        ; $003122
        dc.w    $4E75        ; $003124
        dc.w    $41F8        ; $003126
        dc.w    $9000        ; $003128
        dc.w    $43F9        ; $00312A
        dc.w    $00FF        ; $00312C
        dc.w    $6100        ; $00312E
        dc.w    $3378        ; $003130
        dc.w    $C086        ; $003132
        dc.w    $0002        ; $003134
        dc.w    $4EBA        ; $003136
        dc.w    $F85E        ; $003138
        dc.w    $3038        ; $00313A
        dc.w    $C0AE        ; $00313C
        dc.w    $D169        ; $00313E
        dc.w    $0008        ; $003140
        dc.w    $3038        ; $003142
        dc.w    $C0B0        ; $003144
        dc.w    $D169        ; $003146
        dc.w    $000A        ; $003148
        dc.w    $3038        ; $00314A
        dc.w    $C0B2        ; $00314C
        dc.w    $D169        ; $00314E
        dc.w    $000C        ; $003150
        dc.w    $4A78        ; $003152
        dc.w    $C8C8        ; $003154
        dc.w    $6706        ; $003156
        dc.w    $0669        ; $003158
        dc.w    $00E0        ; $00315A
        dc.w    $0006        ; $00315C
        dc.w    $4E75        ; $00315E
        dc.w    $33FC        ; $003160
        dc.w    $0001        ; $003162
        dc.w    $00FF        ; $003164
        dc.w    $6100        ; $003166
        dc.w    $7070        ; $003168
        dc.w    $D078        ; $00316A
        dc.w    $C0C6        ; $00316C
        dc.w    $33C0        ; $00316E
        dc.w    $00FF        ; $003170
        dc.w    $60CE        ; $003172
        dc.w    $4A78        ; $003174
        dc.w    $C0BA        ; $003176
        dc.w    $672A        ; $003178
        dc.w    $33FC        ; $00317A
        dc.w    $0002        ; $00317C
        dc.w    $00FF        ; $00317E
        dc.w    $6100        ; $003180
        dc.w    $43F8        ; $003182
        dc.w    $C0BA        ; $003184
        dc.w    $45F9        ; $003186
        dc.w    $00FF        ; $003188
        dc.w    $6102        ; $00318A
        dc.w    $34D9        ; $00318C

; --- High-level utility 1 ---
HighLevelUtil1:
        MOVE.W (A1)+,(A2)+        ; $00318E
        MOVE.W (A1)+,(A2)+        ; $003190
        MOVE.W (A1)+,D0        ; $003192
        ASR.W #3,D0        ; $003194
        MOVE.W D0,(A2)+        ; $003196
        MOVE.W (A1)+,D0        ; $003198
        ASR.W #3,D0        ; $00319A
        MOVE.W D0,(A2)+        ; $00319C
        MOVE.W (A1)+,D0        ; $00319E
        ASR.W #3,D0        ; $0031A0
        MOVE.W D0,(A2)        ; $0031A2
        RTS        ; $0031A4
        dc.w    $7000        ; $0031A6
        dc.w    $1038        ; $0031A8
        dc.w    $C305        ; $0031AA
        dc.w    $227B        ; $0031AC
        dc.w    $0004        ; $0031AE
        dc.w    $4ED1        ; $0031B0
        dc.w    $0088        ; $0031B2
        dc.w    $3378        ; $0031B4
        dc.w    $0088        ; $0031B6
        dc.w    $3272        ; $0031B8
        dc.w    $0088        ; $0031BA
        dc.w    $31DE        ; $0031BC
        dc.w    $0088        ; $0031BE
        dc.w    $31DE        ; $0031C0
        dc.w    $0088        ; $0031C2
        dc.w    $31DE        ; $0031C4
        dc.w    $0088        ; $0031C6
        dc.w    $31DE        ; $0031C8
        dc.w    $0088        ; $0031CA
        dc.w    $322A        ; $0031CC
        dc.w    $0088        ; $0031CE
        dc.w    $322A        ; $0031D0
        dc.w    $0088        ; $0031D2
        dc.w    $322A        ; $0031D4
        dc.w    $0088        ; $0031D6
        dc.w    $3204        ; $0031D8
        dc.w    $0088        ; $0031DA
        dc.w    $3250        ; $0031DC
        dc.w    $4A78        ; $0031DE
        dc.w    $C04E        ; $0031E0
        dc.w    $676C        ; $0031E2
        dc.w    $2278        ; $0031E4
        dc.w    $C258        ; $0031E6
        dc.w    $137C        ; $0031E8
        dc.w    $0002        ; $0031EA
        dc.w    $0000        ; $0031EC
        dc.w    $13FC        ; $0031EE
        dc.w    $0003        ; $0031F0
        dc.w    $00FF        ; $0031F2
        dc.w    $6950        ; $0031F4
        dc.w    $13FC        ; $0031F6
        dc.w    $0001        ; $0031F8
        dc.w    $00FF        ; $0031FA
        dc.w    $6940        ; $0031FC
        dc.w    $5838        ; $0031FE
        dc.w    $C305        ; $003200
        dc.w    $4E75        ; $003202
        dc.w    $5338        ; $003204
        dc.w    $C308        ; $003206
        dc.w    $661A        ; $003208
        dc.w    $3038        ; $00320A
        dc.w    $C08E        ; $00320C
        dc.w    $B078        ; $00320E
        dc.w    $C07A        ; $003210
        dc.w    $6710        ; $003212
        dc.w    $3028        ; $003214
        dc.w    $002C        ; $003216
        dc.w    $43F9        ; $003218
        dc.w    $0089        ; $00321A
        dc.w    $89EE        ; $00321C
        dc.w    $11F1        ; $00321E
        dc.w    $0000        ; $003220
        dc.w    $C8A5        ; $003222
        dc.w    $11FC        ; $003224
        dc.w    $0004        ; $003226
        dc.w    $C305        ; $003228
        dc.w    $4A78        ; $00322A
        dc.w    $C04E        ; $00322C
        dc.w    $6720        ; $00322E
        dc.w    $2278        ; $003230
        dc.w    $C258        ; $003232
        dc.w    $137C        ; $003234
        dc.w    $0000        ; $003236
        dc.w    $0000        ; $003238
        dc.w    $13FC        ; $00323A
        dc.w    $0000        ; $00323C
        dc.w    $00FF        ; $00323E
        dc.w    $6940        ; $003240
        dc.w    $13FC        ; $003242
        dc.w    $0000        ; $003244
        dc.w    $00FF        ; $003246
        dc.w    $6950        ; $003248
        dc.w    $5838        ; $00324A
        dc.w    $C305        ; $00324C
        dc.w    $4E75        ; $00324E
        dc.w    $2278        ; $003250
        dc.w    $C258        ; $003252
        dc.w    $137C        ; $003254
        dc.w    $0002        ; $003256
        dc.w    $0000        ; $003258
        dc.w    $13FC        ; $00325A
        dc.w    $0000        ; $00325C
        dc.w    $00FF        ; $00325E
        dc.w    $6940        ; $003260
        dc.w    $13FC        ; $003262
        dc.w    $0000        ; $003264
        dc.w    $00FF        ; $003266
        dc.w    $6950        ; $003268
        dc.w    $11FC        ; $00326A
        dc.w    $0000        ; $00326C
        dc.w    $C305        ; $00326E
        dc.w    $4E75        ; $003270
        dc.w    $11FC        ; $003272
        dc.w    $0002        ; $003274
        dc.w    $C308        ; $003276
        dc.w    $7000        ; $003278
        dc.w    $43F9        ; $00327A
        dc.w    $00FF        ; $00327C
        dc.w    $68D0        ; $00327E
        dc.w    $3028        ; $003280
        dc.w    $002C        ; $003282
        dc.w    $5340        ; $003284
        dc.w    $E948        ; $003286
        dc.w    $D3C0        ; $003288
        dc.w    $21C9        ; $00328A
        dc.w    $C258        ; $00328C
        dc.w    $137C        ; $00328E
        dc.w    $0002        ; $003290
        dc.w    $0000        ; $003292
        dc.w    $13FC        ; $003294
        dc.w    $0003        ; $003296
        dc.w    $00FF        ; $003298
        dc.w    $6950        ; $00329A
        dc.w    $3478        ; $00329C
        dc.w    $C076        ; $00329E
        dc.w    $7000        ; $0032A0
        dc.w    $1038        ; $0032A2
        dc.w    $C806        ; $0032A4
        dc.w    $11FC        ; $0032A6
        dc.w    $0000        ; $0032A8
        dc.w    $C806        ; $0032AA
        dc.w    $D040        ; $0032AC
        dc.w    $47F9        ; $0032AE
        dc.w    $0089        ; $0032B0
        dc.w    $9884        ; $0032B2
        dc.w    $3033        ; $0032B4
        dc.w    $0000        ; $0032B6
        dc.w    $14C0        ; $0032B8
        dc.w    $7000        ; $0032BA
        dc.w    $1038        ; $0032BC
        dc.w    $C807        ; $0032BE
        dc.w    $11FC        ; $0032C0
        dc.w    $00C4        ; $0032C2
        dc.w    $C807        ; $0032C4
        dc.w    $0400        ; $0032C6
        dc.w    $00C4        ; $0032C8
        dc.w    $D040        ; $0032CA
        dc.w    $47F9        ; $0032CC
        dc.w    $0089        ; $0032CE
        dc.w    $9884        ; $0032D0
        dc.w    $3033        ; $0032D2
        dc.w    $0000        ; $0032D4
        dc.w    $14C0        ; $0032D6
        dc.w    $7000        ; $0032D8
        dc.w    $1038        ; $0032DA
        dc.w    $C808        ; $0032DC
        dc.w    $11FC        ; $0032DE
        dc.w    $00C4        ; $0032E0
        dc.w    $C808        ; $0032E2
        dc.w    $0400        ; $0032E4
        dc.w    $00C4        ; $0032E6
        dc.w    $D040        ; $0032E8
        dc.w    $47F9        ; $0032EA
        dc.w    $0089        ; $0032EC
        dc.w    $980C        ; $0032EE
        dc.w    $3033        ; $0032F0
        dc.w    $0000        ; $0032F2
        dc.w    $34C0        ; $0032F4
        dc.w    $31CA        ; $0032F6
        dc.w    $C076        ; $0032F8
        dc.w    $4EB9        ; $0032FA
        dc.w    $0088        ; $0032FC
        dc.w    $B2E4        ; $0032FE
        dc.w    $4EB9        ; $003300
        dc.w    $0088        ; $003302
        dc.w    $B422        ; $003304
        dc.w    $598A        ; $003306
        dc.w    $2012        ; $003308
        dc.w    $23FC        ; $00330A
        dc.w    $222E        ; $00330C
        dc.w    $070C        ; $00330E
        dc.w    $00FF        ; $003310
        dc.w    $6948        ; $003312
        dc.w    $11FC        ; $003314
        dc.w    $0000        ; $003316
        dc.w    $C30B        ; $003318
        dc.w    $B0B8        ; $00331A
        dc.w    $C254        ; $00331C
        dc.w    $6C3C        ; $00331E
        dc.w    $21C0        ; $003320
        dc.w    $C254        ; $003322
        dc.w    $7000        ; $003324
        dc.w    $43F9        ; $003326
        dc.w    $00FF        ; $003328
        dc.w    $68D1        ; $00332A
        dc.w    $1038        ; $00332C
        dc.w    $C307        ; $00332E
        dc.w    $45F1        ; $003330
        dc.w    $0000        ; $003332
        dc.w    $14BC        ; $003334
        dc.w    $0000        ; $003336
        dc.w    $3028        ; $003338
        dc.w    $002C        ; $00333A
        dc.w    $5340        ; $00333C
        dc.w    $E948        ; $00333E
        dc.w    $11C0        ; $003340
        dc.w    $C307        ; $003342
        dc.w    $45F1        ; $003344
        dc.w    $0000        ; $003346
        dc.w    $14BC        ; $003348
        dc.w    $0001        ; $00334A
        dc.w    $23FC        ; $00334C
        dc.w    $222D        ; $00334E
        dc.w    $FB7C        ; $003350
        dc.w    $00FF        ; $003352
        dc.w    $6948        ; $003354
        dc.w    $11FC        ; $003356
        dc.w    $0001        ; $003358
        dc.w    $C30B        ; $00335A
        dc.w    $13FC        ; $00335C
        dc.w    $0001        ; $00335E
        dc.w    $00FF        ; $003360
        dc.w    $6940        ; $003362
        dc.w    $1038        ; $003364
        dc.w    $C8AB        ; $003366
        dc.w    $0200        ; $003368
        dc.w    $0003        ; $00336A
        dc.w    $D000        ; $00336C
        dc.w    $D000        ; $00336E
        dc.w    $0600        ; $003370
        dc.w    $000C        ; $003372
        dc.w    $11C0        ; $003374
        dc.w    $C305        ; $003376
        dc.w    $4E75        ; $003378
        dc.w    $7000        ; $00337A
        dc.w    $1038        ; $00337C
        dc.w    $C305        ; $00337E
        dc.w    $227B        ; $003380
        dc.w    $0004        ; $003382
        dc.w    $4ED1        ; $003384
        dc.w    $0088        ; $003386
        dc.w    $34E6        ; $003388
        dc.w    $0088        ; $00338A
        dc.w    $3404        ; $00338C
        dc.w    $0088        ; $00338E
        dc.w    $33C2        ; $003390
        dc.w    $0088        ; $003392
        dc.w    $33C2        ; $003394
        dc.w    $0088        ; $003396
        dc.w    $33C2        ; $003398
        dc.w    $0088        ; $00339A
        dc.w    $33C2        ; $00339C
        dc.w    $0088        ; $00339E
        dc.w    $33C2        ; $0033A0
        dc.w    $0088        ; $0033A2
        dc.w    $33C2        ; $0033A4
        dc.w    $0088        ; $0033A6
        dc.w    $33C2        ; $0033A8
        dc.w    $0088        ; $0033AA
        dc.w    $33C2        ; $0033AC
        dc.w    $0088        ; $0033AE
        dc.w    $33C2        ; $0033B0
        dc.w    $0088        ; $0033B2
        dc.w    $33C2        ; $0033B4
        dc.w    $0088        ; $0033B6
        dc.w    $33EC        ; $0033B8
        dc.w    $0088        ; $0033BA
        dc.w    $33FC        ; $0033BC
        dc.w    $0088        ; $0033BE
        dc.w    $34CA        ; $0033C0
        dc.w    $3038        ; $0033C2
        dc.w    $C08E        ; $0033C4
        dc.w    $B078        ; $0033C6
        dc.w    $C07A        ; $0033C8
        dc.w    $6718        ; $0033CA
        dc.w    $5838        ; $0033CC
        dc.w    $C305        ; $0033CE
        dc.w    $3028        ; $0033D0
        dc.w    $002C        ; $0033D2
        dc.w    $5340        ; $0033D4
        dc.w    $B038        ; $0033D6
        dc.w    $C310        ; $0033D8
        dc.w    $6606        ; $0033DA
        dc.w    $11FC        ; $0033DC
        dc.w    $0030        ; $0033DE
        dc.w    $C305        ; $0033E0
        dc.w    $4E75        ; $0033E2
        dc.w    $11FC        ; $0033E4
        dc.w    $0034        ; $0033E6
        dc.w    $C305        ; $0033E8
        dc.w    $4E75        ; $0033EA
        dc.w    $3028        ; $0033EC
        dc.w    $002C        ; $0033EE
        dc.w    $43F9        ; $0033F0
        dc.w    $0089        ; $0033F2
        dc.w    $89EE        ; $0033F4
        dc.w    $11F1        ; $0033F6
        dc.w    $0000        ; $0033F8
        dc.w    $C8A5        ; $0033FA
        dc.w    $11FC        ; $0033FC
        dc.w    $0000        ; $0033FE
        dc.w    $C305        ; $003400
        dc.w    $4E75        ; $003402
        dc.w    $43F8        ; $003404
        dc.w    $C806        ; $003406
        dc.w    $B0FC        ; $003408
        dc.w    $9000        ; $00340A
        dc.w    $6704        ; $00340C
        dc.w    $43F8        ; $00340E
        dc.w    $C813        ; $003410
        dc.w    $3478        ; $003412
        dc.w    $C076        ; $003414
        dc.w    $7000        ; $003416
        dc.w    $1011        ; $003418
        dc.w    $12BC        ; $00341A
        dc.w    $0000        ; $00341C
        dc.w    $D040        ; $00341E
        dc.w    $47F9        ; $003420
        dc.w    $0089        ; $003422
        dc.w    $9884        ; $003424
        dc.w    $3033        ; $003426
        dc.w    $0000        ; $003428
        dc.w    $14C0        ; $00342A
        dc.w    $7000        ; $00342C
        dc.w    $1029        ; $00342E
        dc.w    $0001        ; $003430
        dc.w    $137C        ; $003432
        dc.w    $00C4        ; $003434
        dc.w    $0001        ; $003436
        dc.w    $0400        ; $003438
        dc.w    $00C4        ; $00343A
        dc.w    $D040        ; $00343C
        dc.w    $47F9        ; $00343E
        dc.w    $0089        ; $003440
        dc.w    $9884        ; $003442
        dc.w    $3033        ; $003444
        dc.w    $0000        ; $003446
        dc.w    $14C0        ; $003448
        dc.w    $7000        ; $00344A

; --- High-level utility 2 ---
HighLevelUtil2:
        MOVE.B $0002(A1),D0        ; $00344C
        MOVE.B #$00C4,$0002(A1)        ; $003450
        BTST #196,D0        ; $003456
        ADD.W D0,D0        ; $00345A
        LEA $0089980C,A3        ; $00345C
        MOVE.W <EA:33>,D0        ; $003462
        BTST #192,D0        ; $003464
        MOVE.W A2,$C076.W        ; $003468
        JSR $0088B2E4(PC)        ; $00346C
        DC.W $598A ; Unknown        ; $003470
        MOVE.L (A2),D0        ; $003472
        CMP.L $C254.W,D0        ; $003474
        BGE $008834AC        ; $003478
        MOVE.L D0,$C254.W        ; $00347A
        MOVE.W $002C(A0),D0        ; $00347E
        DC.W $5340 ; Unknown        ; $003482
        LSL.W #4,D0        ; $003484
        MOVE.B D0,$C307.W        ; $003486
        DC.W $43F8 ; Unknown        ; $00348A
        AND.W (A4),D1        ; $00348C
        LEA $00FF68E8,A3        ; $00348E
        CMPA.W #$9000,A0        ; $003494
        BEQ $008834A0        ; $003498
        LEA $00FF6968,A3        ; $00349A
        MOVE.B #$0002,$FFF8(A3)        ; $0034A0
        JSR $0088B43C        ; $0034A6
        BTST #5,$C30E.W        ; $0034AC
        BNE $008834C0        ; $0034B2
        TST.B $C819.W        ; $0034B4
        BEQ $008834D2        ; $0034B8
        MOVE.W $C098.W,$C07A.W        ; $0034BA
        MOVE.W #$0004,$C07C.W        ; $0034C0
        SUBQ.B #1,$C819.W        ; $0034C6
        MOVE.B #$0038,$C305.W        ; $0034CA
        RTS        ; $0034D0
        dc.w    $1038        ; $0034D2
        dc.w    $C8AB        ; $0034D4
        dc.w    $0200        ; $0034D6
        dc.w    $0003        ; $0034D8
        dc.w    $D000        ; $0034DA
        dc.w    $D000        ; $0034DC
        dc.w    $0600        ; $0034DE
        dc.w    $000C        ; $0034E0
        dc.w    $11C0        ; $0034E2
        dc.w    $C305        ; $0034E4
        dc.w    $4E75        ; $0034E6
        dc.w    $7000        ; $0034E8
        dc.w    $1038        ; $0034EA
        dc.w    $C305        ; $0034EC
        dc.w    $227B        ; $0034EE
        dc.w    $0004        ; $0034F0
        dc.w    $4ED1        ; $0034F2
        dc.w    $0088        ; $0034F4
        dc.w    $36DC        ; $0034F6
        dc.w    $0088        ; $0034F8
        dc.w    $35B4        ; $0034FA
        dc.w    $0088        ; $0034FC
        dc.w    $3524        ; $0034FE
        dc.w    $0088        ; $003500
        dc.w    $3524        ; $003502
        dc.w    $0088        ; $003504
        dc.w    $3524        ; $003506
        dc.w    $0088        ; $003508
        dc.w    $3524        ; $00350A
        dc.w    $0088        ; $00350C
        dc.w    $3580        ; $00350E
        dc.w    $0088        ; $003510
        dc.w    $3580        ; $003512
        dc.w    $0088        ; $003514
        dc.w    $3580        ; $003516
        dc.w    $0088        ; $003518
        dc.w    $3540        ; $00351A
        dc.w    $0088        ; $00351C
        dc.w    $359C        ; $00351E
        dc.w    $0088        ; $003520
        dc.w    $36C0        ; $003522
        dc.w    $4A78        ; $003524
        dc.w    $C04E        ; $003526
        dc.w    $6772        ; $003528
        dc.w    $13FC        ; $00352A
        dc.w    $0003        ; $00352C
        dc.w    $00FF        ; $00352E
        dc.w    $6950        ; $003530
        dc.w    $13FC        ; $003532
        dc.w    $0001        ; $003534
        dc.w    $00FF        ; $003536
        dc.w    $6940        ; $003538
        dc.w    $5838        ; $00353A
        dc.w    $C305        ; $00353C
        dc.w    $4E75        ; $00353E
        dc.w    $5338        ; $003540
        dc.w    $C308        ; $003542
        dc.w    $6634        ; $003544
        dc.w    $3038        ; $003546
        dc.w    $C08E        ; $003548
        dc.w    $B078        ; $00354A
        dc.w    $C07A        ; $00354C
        dc.w    $672A        ; $00354E
        dc.w    $0838        ; $003550
        dc.w    $0005        ; $003552
        dc.w    $C30E        ; $003554
        dc.w    $6622        ; $003556
        dc.w    $3028        ; $003558
        dc.w    $002C        ; $00355A
        dc.w    $43F9        ; $00355C
        dc.w    $0089        ; $00355E
        dc.w    $89EE        ; $003560
        dc.w    $11F1        ; $003562
        dc.w    $0000        ; $003564
        dc.w    $C8A5        ; $003566
        dc.w    $0CB9        ; $003568
        dc.w    $222E        ; $00356A
        dc.w    $0508        ; $00356C
        dc.w    $00FF        ; $00356E
        dc.w    $6948        ; $003570
        dc.w    $6606        ; $003572
        dc.w    $11FC        ; $003574
        dc.w    $0097        ; $003576
        dc.w    $C8A5        ; $003578
        dc.w    $11FC        ; $00357A
        dc.w    $0004        ; $00357C
        dc.w    $C305        ; $00357E
        dc.w    $4A78        ; $003580
        dc.w    $C04E        ; $003582
        dc.w    $6716        ; $003584
        dc.w    $13FC        ; $003586
        dc.w    $0000        ; $003588
        dc.w    $00FF        ; $00358A
        dc.w    $6940        ; $00358C
        dc.w    $13FC        ; $00358E
        dc.w    $0000        ; $003590
        dc.w    $00FF        ; $003592
        dc.w    $6950        ; $003594
        dc.w    $5838        ; $003596
        dc.w    $C305        ; $003598
        dc.w    $4E75        ; $00359A
        dc.w    $13FC        ; $00359C
        dc.w    $0000        ; $00359E
        dc.w    $00FF        ; $0035A0
        dc.w    $6940        ; $0035A2
        dc.w    $13FC        ; $0035A4
        dc.w    $0000        ; $0035A6
        dc.w    $00FF        ; $0035A8
        dc.w    $6950        ; $0035AA
        dc.w    $11FC        ; $0035AC
        dc.w    $0000        ; $0035AE
        dc.w    $C305        ; $0035B0
        dc.w    $4E75        ; $0035B2
        dc.w    $11FC        ; $0035B4
        dc.w    $0001        ; $0035B6
        dc.w    $C308        ; $0035B8
        dc.w    $3028        ; $0035BA
        dc.w    $002C        ; $0035BC
        dc.w    $5240        ; $0035BE
        dc.w    $B038        ; $0035C0
        dc.w    $C310        ; $0035C2
        dc.w    $6606        ; $0035C4
        dc.w    $11FC        ; $0035C6
        dc.w    $0002        ; $0035C8
        dc.w    $C308        ; $0035CA
        dc.w    $13FC        ; $0035CC
        dc.w    $0003        ; $0035CE
        dc.w    $00FF        ; $0035D0
        dc.w    $6950        ; $0035D2
        dc.w    $3478        ; $0035D4
        dc.w    $C076        ; $0035D6
        dc.w    $7000        ; $0035D8
        dc.w    $1038        ; $0035DA
        dc.w    $C806        ; $0035DC
        dc.w    $11FC        ; $0035DE
        dc.w    $0000        ; $0035E0
        dc.w    $C806        ; $0035E2
        dc.w    $D040        ; $0035E4
        dc.w    $47F9        ; $0035E6
        dc.w    $0089        ; $0035E8
        dc.w    $9884        ; $0035EA
        dc.w    $3033        ; $0035EC
        dc.w    $0000        ; $0035EE
        dc.w    $14C0        ; $0035F0
        dc.w    $7000        ; $0035F2
        dc.w    $1038        ; $0035F4
        dc.w    $C807        ; $0035F6
        dc.w    $11FC        ; $0035F8
        dc.w    $00C4        ; $0035FA
        dc.w    $C807        ; $0035FC
        dc.w    $0400        ; $0035FE
        dc.w    $00C4        ; $003600
        dc.w    $D040        ; $003602
        dc.w    $47F9        ; $003604
        dc.w    $0089        ; $003606
        dc.w    $9884        ; $003608
        dc.w    $3033        ; $00360A
        dc.w    $0000        ; $00360C
        dc.w    $14C0        ; $00360E
        dc.w    $7000        ; $003610
        dc.w    $1038        ; $003612
        dc.w    $C808        ; $003614
        dc.w    $11FC        ; $003616
        dc.w    $00C4        ; $003618
        dc.w    $C808        ; $00361A
        dc.w    $0400        ; $00361C
        dc.w    $00C4        ; $00361E
        dc.w    $D040        ; $003620
        dc.w    $47F9        ; $003622
        dc.w    $0089        ; $003624
        dc.w    $980C        ; $003626
        dc.w    $3033        ; $003628
        dc.w    $0000        ; $00362A
        dc.w    $34C0        ; $00362C
        dc.w    $31CA        ; $00362E
        dc.w    $C076        ; $003630
        dc.w    $4EBA        ; $003632
        dc.w    $7CB0        ; $003634
        dc.w    $4EBA        ; $003636
        dc.w    $7DD6        ; $003638
        dc.w    $598A        ; $00363A
        dc.w    $2012        ; $00363C
        dc.w    $23FC        ; $00363E
        dc.w    $222E        ; $003640
        dc.w    $070C        ; $003642
        dc.w    $00FF        ; $003644
        dc.w    $6948        ; $003646
        dc.w    $B0B8        ; $003648
        dc.w    $C254        ; $00364A
        dc.w    $6C62        ; $00364C
        dc.w    $21C0        ; $00364E
        dc.w    $C254        ; $003650
        dc.w    $7200        ; $003652
        dc.w    $3228        ; $003654
        dc.w    $002C        ; $003656
        dc.w    $5341        ; $003658
        dc.w    $E949        ; $00365A
        dc.w    $11C1        ; $00365C
        dc.w    $C307        ; $00365E
        dc.w    $23FC        ; $003660
        dc.w    $222D        ; $003662
        dc.w    $FB7C        ; $003664
        dc.w    $00FF        ; $003666
        dc.w    $6948        ; $003668
        dc.w    $43F8        ; $00366A
        dc.w    $FDAA        ; $00366C
        dc.w    $3238        ; $00366E
        dc.w    $C89C        ; $003670
        dc.w    $EB49        ; $003672
        dc.w    $D278        ; $003674
        dc.w    $C8A0        ; $003676
        dc.w    $3438        ; $003678
        dc.w    $C8C8        ; $00367A
        dc.w    $E74A        ; $00367C
        dc.w    $D478        ; $00367E
        dc.w    $C8CC        ; $003680
        dc.w    $D242        ; $003682
        dc.w    $43F1        ; $003684
        dc.w    $1000        ; $003686
        dc.w    $B091        ; $003688
        dc.w    $6C24        ; $00368A
        dc.w    $23FC        ; $00368C
        dc.w    $222E        ; $00368E
        dc.w    $0508        ; $003690
        dc.w    $00FF        ; $003692
        dc.w    $6948        ; $003694
        dc.w    $22C0        ; $003696
        dc.w    $22F8        ; $003698
        dc.w    $C270        ; $00369A
        dc.w    $22B8        ; $00369C
        dc.w    $C274        ; $00369E
        dc.w    $43F9        ; $0036A0
        dc.w    $00FF        ; $0036A2
        dc.w    $6958        ; $0036A4
        dc.w    $45F9        ; $0036A6
        dc.w    $00FF        ; $0036A8
        dc.w    $68E8        ; $0036AA
        dc.w    $24D9        ; $0036AC
        dc.w    $2491        ; $0036AE
        dc.w    $13FC        ; $0036B0
        dc.w    $0001        ; $0036B2
        dc.w    $00FF        ; $0036B4
        dc.w    $6940        ; $0036B6
        dc.w    $0838        ; $0036B8
        dc.w    $0005        ; $0036BA
        dc.w    $C30E        ; $0036BC
        dc.w    $6708        ; $0036BE
        dc.w    $11FC        ; $0036C0
        dc.w    $002C        ; $0036C2
        dc.w    $C305        ; $0036C4
        dc.w    $4E75        ; $0036C6
        dc.w    $1038        ; $0036C8
        dc.w    $C8AB        ; $0036CA
        dc.w    $0200        ; $0036CC
        dc.w    $0003        ; $0036CE
        dc.w    $D000        ; $0036D0
        dc.w    $D000        ; $0036D2
        dc.w    $0600        ; $0036D4
        dc.w    $000C        ; $0036D6
        dc.w    $11C0        ; $0036D8
        dc.w    $C305        ; $0036DA
        dc.w    $4E75        ; $0036DC
        dc.w    $41F8        ; $0036DE
        dc.w    $9100        ; $0036E0
        dc.w    $43F9        ; $0036E2
        dc.w    $00FF        ; $0036E4
        dc.w    $6218        ; $0036E6
        dc.w    $47F9        ; $0036E8
        dc.w    $0089        ; $0036EA
        dc.w    $58E4        ; $0036EC
        dc.w    $3038        ; $0036EE
        dc.w    $C8CC        ; $0036F0
        dc.w    $2673        ; $0036F2
        dc.w    $0000        ; $0036F4
        dc.w    $7E0E        ; $0036F6
        dc.w    $7A00        ; $0036F8
        dc.w    $7C00        ; $0036FA
        dc.w    $7000        ; $0036FC
        dc.w    $1028        ; $0036FE
        dc.w    $00C1        ; $003700
        dc.w    $6700        ; $003702
        dc.w    $008C        ; $003704
        dc.w    $7A01        ; $003706
        dc.w    $7C01        ; $003708
        dc.w    $4A38        ; $00370A
        dc.w    $90E4        ; $00370C
        dc.w    $6606        ; $00370E
        dc.w    $4A38        ; $003710
        dc.w    $C31C        ; $003712
        dc.w    $6710        ; $003714
        dc.w    $0828        ; $003716
        dc.w    $0003        ; $003718
        dc.w    $00E5        ; $00371A
        dc.w    $6716        ; $00371C
        dc.w    $7A00        ; $00371E
        dc.w    $7C00        ; $003720
        dc.w    $6000        ; $003722
        dc.w    $006C        ; $003724
        dc.w    $0838        ; $003726
        dc.w    $0003        ; $003728
        dc.w    $90E5        ; $00372A
        dc.w    $6706        ; $00372C
        dc.w    $4A28        ; $00372E
        dc.w    $00E4        ; $003730
        dc.w    $66EA        ; $003732
        dc.w    $0C40        ; $003734
        dc.w    $0001        ; $003736
        dc.w    $6702        ; $003738
        dc.w    $7C00        ; $00373A
        dc.w    $D040        ; $00373C
        dc.w    $D040        ; $00373E
        dc.w    $D068        ; $003740
        dc.w    $00C2        ; $003742
        dc.w    $2373        ; $003744
        dc.w    $0000        ; $003746
        dc.w    $0010        ; $003748
        dc.w    $3038        ; $00374A
        dc.w    $C0E4        ; $00374C
        dc.w    $D068        ; $00374E
        dc.w    $0032        ; $003750
        dc.w    $3340        ; $003752
        dc.w    $0004        ; $003754
        dc.w    $3028        ; $003756
        dc.w    $003A        ; $003758
        dc.w    $E640        ; $00375A
        dc.w    $4440        ; $00375C
        dc.w    $3340        ; $00375E
        dc.w    $0008        ; $003760
        dc.w    $3028        ; $003762
        dc.w    $003C        ; $003764
        dc.w    $D068        ; $003766
        dc.w    $006E        ; $003768
        dc.w    $E640        ; $00376A
        dc.w    $3340        ; $00376C
        dc.w    $000A        ; $00376E
        dc.w    $3028        ; $003770
        dc.w    $003E        ; $003772
        dc.w    $E640        ; $003774
        dc.w    $4440        ; $003776
        dc.w    $3340        ; $003778
        dc.w    $000C        ; $00377A
        dc.w    $3028        ; $00377C
        dc.w    $00BC        ; $00377E
        dc.w    $E640        ; $003780
        dc.w    $3340        ; $003782
        dc.w    $001C        ; $003784
        dc.w    $3028        ; $003786
        dc.w    $00C4        ; $003788
        dc.w    $E640        ; $00378A
        dc.w    $3340        ; $00378C
        dc.w    $0030        ; $00378E
        dc.w    $3368        ; $003790
        dc.w    $0030        ; $003792
        dc.w    $0002        ; $003794
        dc.w    $3368        ; $003796
        dc.w    $0034        ; $003798
        dc.w    $0006        ; $00379A
        dc.w    $3345        ; $00379C
        dc.w    $0000        ; $00379E
        dc.w    $3346        ; $0037A0
        dc.w    $0014        ; $0037A2
        dc.w    $3346        ; $0037A4
        dc.w    $0028        ; $0037A6
        dc.w    $41E8        ; $0037A8
        dc.w    $0100        ; $0037AA
        dc.w    $43E9        ; $0037AC
        dc.w    $003C        ; $0037AE
        dc.w    $51CF        ; $0037B0
        dc.w    $FF46        ; $0037B2
        dc.w    $4E75        ; $0037B4
        dc.w    $41F8        ; $0037B6
        dc.w    $9000        ; $0037B8
        dc.w    $3238        ; $0037BA
        dc.w    $C8A0        ; $0037BC
        dc.w    $43F9        ; $0037BE
        dc.w    $0089        ; $0037C0
        dc.w    $5A64        ; $0037C2
        dc.w    $2271        ; $0037C4
        dc.w    $1000        ; $0037C6
        dc.w    $45F9        ; $0037C8
        dc.w    $00FF        ; $0037CA
        dc.w    $659C        ; $0037CC
        dc.w    $323C        ; $0037CE
        dc.w    $0C80        ; $0037D0
        dc.w    $7E02        ; $0037D2
        dc.w    $3428        ; $0037D4
        dc.w    $0030        ; $0037D6
        dc.w    $3828        ; $0037D8
        dc.w    $0034        ; $0037DA
        dc.w    $9451        ; $0037DC
        dc.w    $6A02        ; $0037DE
        dc.w    $4442        ; $0037E0
        dc.w    $B441        ; $0037E2
        dc.w    $6E46        ; $0037E4
        dc.w    $9869        ; $0037E6
        dc.w    $0004        ; $0037E8
        dc.w    $6A02        ; $0037EA
        dc.w    $4444        ; $0037EC
        dc.w    $B841        ; $0037EE
        dc.w    $6E3A        ; $0037F0
        dc.w    $357C        ; $0037F2
        dc.w    $0001        ; $0037F4
        dc.w    $0000        ; $0037F6
        dc.w    $2559        ; $0037F8
        dc.w    $0002        ; $0037FA
        dc.w    $3559        ; $0037FC
        dc.w    $0006        ; $0037FE
        dc.w    $3559        ; $003800
        dc.w    $000A        ; $003802
        dc.w    $3559        ; $003804
        dc.w    $000E        ; $003806
        dc.w    $2251        ; $003808
        dc.w    $3038        ; $00380A
        dc.w    $C008        ; $00380C
        dc.w    $5240        ; $00380E
        dc.w    $0C40        ; $003810
        dc.w    $000C        ; $003812
        dc.w    $6604        ; $003814
        dc.w    $303C        ; $003816
        dc.w    $0000        ; $003818
        dc.w    $31C0        ; $00381A
        dc.w    $C008        ; $00381C
        dc.w    $E248        ; $00381E
        dc.w    $D040        ; $003820
        dc.w    $D040        ; $003822
        dc.w    $2571        ; $003824
        dc.w    $0000        ; $003826
        dc.w    $0010        ; $003828
        dc.w    $600E        ; $00382A
        dc.w    $43E9        ; $00382C
        dc.w    $000E        ; $00382E
        dc.w    $51CF        ; $003830
        dc.w    $FFA2        ; $003832
        dc.w    $357C        ; $003834
        dc.w    $0000        ; $003836
        dc.w    $0000        ; $003838
        dc.w    $3038        ; $00383A
        dc.w    $C8A0        ; $00383C
        dc.w    $227B        ; $00383E
        dc.w    $0004        ; $003840
        dc.w    $4ED1        ; $003842
        dc.w    $0088        ; $003844
        dc.w    $385E        ; $003846
        dc.w    $0088        ; $003848
        dc.w    $395C        ; $00384A
        dc.w    $0088        ; $00384C
        dc.w    $385C        ; $00384E
        dc.w    $0088        ; $003850
        dc.w    $3AAA        ; $003852
        dc.w    $0088        ; $003854
        dc.w    $385C        ; $003856
        dc.w    $0088        ; $003858
        dc.w    $385C        ; $00385A
        dc.w    $4E75        ; $00385C
        dc.w    $4A38        ; $00385E
        dc.w    $C80F        ; $003860
        dc.w    $6702        ; $003862
        dc.w    $4E75        ; $003864
        dc.w    $43F9        ; $003866
        dc.w    $0088        ; $003868
        dc.w    $3924        ; $00386A
        dc.w    $45F9        ; $00386C
        dc.w    $00FF        ; $00386E
        dc.w    $65B0        ; $003870
        dc.w    $5678        ; $003872
        dc.w    $C8E2        ; $003874
        dc.w    $323C        ; $003876
        dc.w    $0C80        ; $003878
        dc.w    $363C        ; $00387A
        dc.w    $1400        ; $00387C
        dc.w    $357C        ; $00387E
        dc.w    $0000        ; $003880
        dc.w    $0000        ; $003882
        dc.w    $357C        ; $003884
        dc.w    $0000        ; $003886
        dc.w    $0014        ; $003888
        dc.w    $3038        ; $00388A
        dc.w    $C8E2        ; $00388C
        dc.w    $0240        ; $00388E
        dc.w    $1FFF        ; $003890
        dc.w    $3540        ; $003892
        dc.w    $0020        ; $003894
        dc.w    $6128        ; $003896
        dc.w    $45EA        ; $003898
        dc.w    $0028        ; $00389A
        dc.w    $3038        ; $00389C
        dc.w    $C8E2        ; $00389E
        dc.w    $E148        ; $0038A0
        dc.w    $48E7        ; $0038A2
        dc.w    $4040        ; $0038A4
        dc.w    $4EBA        ; $0038A6
        dc.w    $56AA        ; $0038A8
        dc.w    $4CDF        ; $0038AA
        dc.w    $0202        ; $0038AC
        dc.w    $E540        ; $0038AE
        dc.w    $3540        ; $0038B0
        dc.w    $0020        ; $0038B2
        dc.w    $357C        ; $0038B4
        dc.w    $0000        ; $0038B6
        dc.w    $0000        ; $0038B8
        dc.w    $357C        ; $0038BA
        dc.w    $0000        ; $0038BC
        dc.w    $0010        ; $0038BE

; --- High-level utility 3 ---
HighLevelUtil3:
        MOVE.W $0030(A0),D2        ; $0038C0
        MOVE.W $0034(A0),D4        ; $0038C4
        MOVE.W $0032(A0),D5        ; $0038C8
        SUB.W (A1),D2        ; $0038CC
        BPL $008838D2        ; $0038CE
        NEG.W D2        ; $0038D0
        CMP.W D1,D2        ; $0038D2
        BGT $00883922        ; $0038D4
        SUB.W $0002(A1),D5        ; $0038D6
        BPL $008838DE        ; $0038DA
        NEG.W D5        ; $0038DC
        CMP.W D3,D5        ; $0038DE
        BGT $00883922        ; $0038E0
        SUB.W $0004(A1),D4        ; $0038E2
        BPL $008838EA        ; $0038E6
        NEG.W D4        ; $0038E8
        CMP.W D1,D4        ; $0038EA
        BGT $00883922        ; $0038EC
        MOVE.W #$0002,$0000(A2)        ; $0038EE
        MOVE.L (A1)+,$0002(A2)        ; $0038F4
        MOVE.W (A1)+,$0006(A2)        ; $0038F8
        MOVE.W (A1)+,$000A(A2)        ; $0038FC
        MOVE.W (A1)+,$000E(A2)        ; $003900
        MOVE.L (A1)+,$0010(A2)        ; $003904
        MOVE.W #$0002,$0014(A2)        ; $003908
        MOVE.L (A1)+,$0016(A2)        ; $00390E
        MOVE.W (A1)+,$001A(A2)        ; $003912
        MOVE.W (A1)+,$001E(A2)        ; $003916
        MOVE.W (A1)+,$0022(A2)        ; $00391A
        MOVE.L (A1)+,$0024(A2)        ; $00391E
        RTS        ; $003922
        dc.w    $F809        ; $003924
        dc.w    $1417        ; $003926
        dc.w    $CF1F        ; $003928
        dc.w    $F07F        ; $00392A
        dc.w    $0000        ; $00392C
        dc.w    $222A        ; $00392E
        dc.w    $1498        ; $003930
        dc.w    $0000        ; $003932
        dc.w    $1CB3        ; $003934
        dc.w    $0000        ; $003936
        dc.w    $0000        ; $003938
        dc.w    $0000        ; $00393A
        dc.w    $222A        ; $00393C
        dc.w    $160C        ; $00393E
        dc.w    $FDF0        ; $003940
        dc.w    $0A2C        ; $003942
        dc.w    $D161        ; $003944
        dc.w    $1000        ; $003946
        dc.w    $0000        ; $003948
        dc.w    $222A        ; $00394A
        dc.w    $1DBE        ; $00394C
        dc.w    $0000        ; $00394E
        dc.w    $1A69        ; $003950
        dc.w    $0000        ; $003952
        dc.w    $0000        ; $003954
        dc.w    $0000        ; $003956
        dc.w    $222A        ; $003958
        dc.w    $1EB8        ; $00395A
        dc.w    $43F9        ; $00395C
        dc.w    $0088        ; $00395E
        dc.w    $3A4E        ; $003960
        dc.w    $45F9        ; $003962
        dc.w    $00FF        ; $003964
        dc.w    $65B0        ; $003966
        dc.w    $203C        ; $003968
        dc.w    $2229        ; $00396A
        dc.w    $5A24        ; $00396C
        dc.w    $7E03        ; $00396E
        dc.w    $4EBA        ; $003970
        dc.w    $007A        ; $003972
        dc.w    $4A38        ; $003974
        dc.w    $C80F        ; $003976
        dc.w    $6702        ; $003978
        dc.w    $4E75        ; $00397A
        dc.w    $43F9        ; $00397C
        dc.w    $0088        ; $00397E
        dc.w    $3A80        ; $003980
        dc.w    $45F9        ; $003982
        dc.w    $00FF        ; $003984
        dc.w    $65D8        ; $003986
        dc.w    $0678        ; $003988
        dc.w    $001E        ; $00398A
        dc.w    $C8E2        ; $00398C
        dc.w    $323C        ; $00398E
        dc.w    $0C80        ; $003990
        dc.w    $363C        ; $003992
        dc.w    $1400        ; $003994
        dc.w    $357C        ; $003996
        dc.w    $0000        ; $003998
        dc.w    $0000        ; $00399A
        dc.w    $357C        ; $00399C
        dc.w    $0000        ; $00399E
        dc.w    $0014        ; $0039A0
        dc.w    $3038        ; $0039A2
        dc.w    $C8E2        ; $0039A4
        dc.w    $0240        ; $0039A6
        dc.w    $1FFF        ; $0039A8
        dc.w    $3540        ; $0039AA
        dc.w    $0020        ; $0039AC
        dc.w    $4EBA        ; $0039AE
        dc.w    $FF10        ; $0039B0
        dc.w    $45F9        ; $0039B2
        dc.w    $00FF        ; $0039B4
        dc.w    $65C4        ; $0039B6
        dc.w    $357C        ; $0039B8
        dc.w    $0000        ; $0039BA
        dc.w    $0000        ; $0039BC
        dc.w    $4A79        ; $0039BE
        dc.w    $00FF        ; $0039C0
        dc.w    $65D8        ; $0039C2
        dc.w    $6724        ; $0039C4
        dc.w    $43F9        ; $0039C6
        dc.w    $0088        ; $0039C8
        dc.w    $3A76        ; $0039CA
        dc.w    $357C        ; $0039CC
        dc.w    $0001        ; $0039CE
        dc.w    $0000        ; $0039D0
        dc.w    $2559        ; $0039D2
        dc.w    $0002        ; $0039D4
        dc.w    $3559        ; $0039D6
        dc.w    $0006        ; $0039D8
        dc.w    $3559        ; $0039DA
        dc.w    $000A        ; $0039DC
        dc.w    $3551        ; $0039DE
        dc.w    $000E        ; $0039E0
        dc.w    $257C        ; $0039E2
        dc.w    $222A        ; $0039E4
        dc.w    $218E        ; $0039E6
        dc.w    $0010        ; $0039E8
        dc.w    $4E75        ; $0039EA
        dc.w    $323C        ; $0039EC
        dc.w    $0C80        ; $0039EE
        dc.w    $363C        ; $0039F0
        dc.w    $0300        ; $0039F2
        dc.w    $3428        ; $0039F4
        dc.w    $0030        ; $0039F6
        dc.w    $3828        ; $0039F8
        dc.w    $0034        ; $0039FA
        dc.w    $3A28        ; $0039FC
        dc.w    $0032        ; $0039FE
        dc.w    $9451        ; $003A00
        dc.w    $6A02        ; $003A02
        dc.w    $4442        ; $003A04
        dc.w    $B441        ; $003A06
        dc.w    $6E34        ; $003A08
        dc.w    $9A69        ; $003A0A
        dc.w    $0002        ; $003A0C
        dc.w    $6A02        ; $003A0E
        dc.w    $4445        ; $003A10
        dc.w    $BA43        ; $003A12
        dc.w    $6E28        ; $003A14
        dc.w    $9869        ; $003A16
        dc.w    $0004        ; $003A18
        dc.w    $6A02        ; $003A1A
        dc.w    $4444        ; $003A1C
        dc.w    $B841        ; $003A1E
        dc.w    $6E1C        ; $003A20
        dc.w    $357C        ; $003A22
        dc.w    $0001        ; $003A24
        dc.w    $0000        ; $003A26
        dc.w    $2559        ; $003A28
        dc.w    $0002        ; $003A2A
        dc.w    $3559        ; $003A2C
        dc.w    $0006        ; $003A2E
        dc.w    $3559        ; $003A30
        dc.w    $000A        ; $003A32
        dc.w    $3551        ; $003A34
        dc.w    $000E        ; $003A36
        dc.w    $2540        ; $003A38
        dc.w    $0010        ; $003A3A
        dc.w    $4E75        ; $003A3C
        dc.w    $43E9        ; $003A3E
        dc.w    $000A        ; $003A40
        dc.w    $51CF        ; $003A42
        dc.w    $FFB0        ; $003A44
        dc.w    $357C        ; $003A46
        dc.w    $0000        ; $003A48
        dc.w    $0000        ; $003A4A
        dc.w    $4E75        ; $003A4C
        dc.w    $F372        ; $003A4E
        dc.w    $1E33        ; $003A50
        dc.w    $EE67        ; $003A52
        dc.w    $F000        ; $003A54
        dc.w    $0100        ; $003A56
        dc.w    $1049        ; $003A58
        dc.w    $0FCD        ; $003A5A
        dc.w    $E5E2        ; $003A5C
        dc.w    $F000        ; $003A5E
        dc.w    $0110        ; $003A60
        dc.w    $F0A8        ; $003A62
        dc.w    $0DE6        ; $003A64
        dc.w    $E977        ; $003A66
        dc.w    $FCDF        ; $003A68
        dc.w    $00F7        ; $003A6A
        dc.w    $0F89        ; $003A6C
        dc.w    $08CD        ; $003A6E
        dc.w    $E03C        ; $003A70
        dc.w    $F800        ; $003A72
        dc.w    $0100        ; $003A74
        dc.w    $1617        ; $003A76
        dc.w    $06AC        ; $003A78
        dc.w    $F7AE        ; $003A7A
        dc.w    $F476        ; $003A7C
        dc.w    $0100        ; $003A7E
        dc.w    $1579        ; $003A80
        dc.w    $10A8        ; $003A82
        dc.w    $F74A        ; $003A84
        dc.w    $F447        ; $003A86
        dc.w    $0000        ; $003A88
        dc.w    $222A        ; $003A8A
        dc.w    $20DE        ; $003A8C
        dc.w    $0000        ; $003A8E
        dc.w    $0000        ; $003A90
        dc.w    $FA6A        ; $003A92
        dc.w    $0000        ; $003A94
        dc.w    $0000        ; $003A96
        dc.w    $222A        ; $003A98
        dc.w    $2272        ; $003A9A
        dc.w    $F2E1        ; $003A9C
        dc.w    $0633        ; $003A9E
        dc.w    $2A89        ; $003AA0
        dc.w    $0000        ; $003AA2
        dc.w    $0100        ; $003AA4
        dc.w    $222A        ; $003AA6
        dc.w    $2508        ; $003AA8
        dc.w    $4A38        ; $003AAA
        dc.w    $C80F        ; $003AAC
        dc.w    $6702        ; $003AAE
        dc.w    $4E75        ; $003AB0
        dc.w    $43F9        ; $003AB2
        dc.w    $0088        ; $003AB4
        dc.w    $3A9C        ; $003AB6
        dc.w    $45F9        ; $003AB8
        dc.w    $00FF        ; $003ABA
        dc.w    $65B0        ; $003ABC
        dc.w    $0678        ; $003ABE
        dc.w    $003C        ; $003AC0
        dc.w    $C8E2        ; $003AC2
        dc.w    $323C        ; $003AC4
        dc.w    $0C80        ; $003AC6
        dc.w    $363C        ; $003AC8
        dc.w    $1400        ; $003ACA
        dc.w    $357C        ; $003ACC
        dc.w    $0000        ; $003ACE
        dc.w    $0000        ; $003AD0
        dc.w    $3038        ; $003AD2
        dc.w    $C8E2        ; $003AD4
        dc.w    $0240        ; $003AD6
        dc.w    $1FFF        ; $003AD8
        dc.w    $3540        ; $003ADA
        dc.w    $000A        ; $003ADC
        dc.w    $3428        ; $003ADE
        dc.w    $0030        ; $003AE0
        dc.w    $3828        ; $003AE2
        dc.w    $0034        ; $003AE4
        dc.w    $3A28        ; $003AE6
        dc.w    $0032        ; $003AE8
        dc.w    $9451        ; $003AEA
        dc.w    $6A02        ; $003AEC
        dc.w    $4442        ; $003AEE
        dc.w    $B441        ; $003AF0
        dc.w    $6E32        ; $003AF2
        dc.w    $9A69        ; $003AF4
        dc.w    $0002        ; $003AF6
        dc.w    $6A02        ; $003AF8
        dc.w    $4445        ; $003AFA
        dc.w    $BA43        ; $003AFC
        dc.w    $6E26        ; $003AFE
        dc.w    $9869        ; $003B00
        dc.w    $0004        ; $003B02
        dc.w    $6A02        ; $003B04
        dc.w    $4444        ; $003B06
        dc.w    $B841        ; $003B08
        dc.w    $6E1A        ; $003B0A
        dc.w    $357C        ; $003B0C
        dc.w    $0001        ; $003B0E
        dc.w    $0000        ; $003B10
        dc.w    $2559        ; $003B12
        dc.w    $0002        ; $003B14
        dc.w    $3559        ; $003B16
        dc.w    $0006        ; $003B18
        dc.w    $3559        ; $003B1A
        dc.w    $000C        ; $003B1C
        dc.w    $3559        ; $003B1E
        dc.w    $000E        ; $003B20
        dc.w    $2559        ; $003B22
        dc.w    $0010        ; $003B24
        dc.w    $4E75        ; $003B26
        dc.w    $41F8        ; $003B28
        dc.w    $9F00        ; $003B2A
        dc.w    $3238        ; $003B2C
        dc.w    $C8A0        ; $003B2E
        dc.w    $43F9        ; $003B30
        dc.w    $0089        ; $003B32
        dc.w    $5A64        ; $003B34
        dc.w    $2271        ; $003B36
        dc.w    $1000        ; $003B38
        dc.w    $45F9        ; $003B3A
        dc.w    $00FF        ; $003B3C
        dc.w    $663C        ; $003B3E
        dc.w    $323C        ; $003B40
        dc.w    $0C80        ; $003B42
        dc.w    $7E02        ; $003B44
        dc.w    $3428        ; $003B46
        dc.w    $0030        ; $003B48
        dc.w    $3828        ; $003B4A
        dc.w    $0034        ; $003B4C
        dc.w    $9451        ; $003B4E
        dc.w    $6A02        ; $003B50
        dc.w    $4442        ; $003B52
        dc.w    $B441        ; $003B54
        dc.w    $6E46        ; $003B56
        dc.w    $9869        ; $003B58
        dc.w    $0004        ; $003B5A
        dc.w    $6A02        ; $003B5C
        dc.w    $4444        ; $003B5E
        dc.w    $B841        ; $003B60
        dc.w    $6E3A        ; $003B62
        dc.w    $357C        ; $003B64
        dc.w    $0001        ; $003B66
        dc.w    $0000        ; $003B68
        dc.w    $2559        ; $003B6A
        dc.w    $0002        ; $003B6C
        dc.w    $3559        ; $003B6E
        dc.w    $0006        ; $003B70
        dc.w    $3559        ; $003B72
        dc.w    $000A        ; $003B74
        dc.w    $3559        ; $003B76
        dc.w    $000E        ; $003B78
        dc.w    $2251        ; $003B7A
        dc.w    $3038        ; $003B7C
        dc.w    $C008        ; $003B7E
        dc.w    $5240        ; $003B80
        dc.w    $0C40        ; $003B82
        dc.w    $000C        ; $003B84
        dc.w    $6604        ; $003B86
        dc.w    $303C        ; $003B88
        dc.w    $0000        ; $003B8A
        dc.w    $31C0        ; $003B8C
        dc.w    $C008        ; $003B8E
        dc.w    $E248        ; $003B90
        dc.w    $D040        ; $003B92
        dc.w    $D040        ; $003B94
        dc.w    $2571        ; $003B96
        dc.w    $0000        ; $003B98
        dc.w    $0010        ; $003B9A
        dc.w    $600E        ; $003B9C
        dc.w    $43E9        ; $003B9E
        dc.w    $000E        ; $003BA0
        dc.w    $51CF        ; $003BA2
        dc.w    $FFA2        ; $003BA4
        dc.w    $357C        ; $003BA6
        dc.w    $0000        ; $003BA8
        dc.w    $0000        ; $003BAA
        dc.w    $3238        ; $003BAC
        dc.w    $C89C        ; $003BAE
        dc.w    $0C41        ; $003BB0
        dc.w    $0001        ; $003BB2
        dc.w    $6672        ; $003BB4
        dc.w    $43F9        ; $003BB6
        dc.w    $0088        ; $003BB8
        dc.w    $3A4E        ; $003BBA
        dc.w    $45F9        ; $003BBC
        dc.w    $00FF        ; $003BBE
        dc.w    $6650        ; $003BC0
        dc.w    $323C        ; $003BC2
        dc.w    $0C80        ; $003BC4
        dc.w    $363C        ; $003BC6
        dc.w    $0300        ; $003BC8
        dc.w    $7E03        ; $003BCA
        dc.w    $3428        ; $003BCC
        dc.w    $0030        ; $003BCE
        dc.w    $3828        ; $003BD0
        dc.w    $0034        ; $003BD2
        dc.w    $3A28        ; $003BD4
        dc.w    $0032        ; $003BD6
        dc.w    $9451        ; $003BD8
        dc.w    $6A02        ; $003BDA
        dc.w    $4442        ; $003BDC
        dc.w    $B441        ; $003BDE
        dc.w    $6E38        ; $003BE0
        dc.w    $9A69        ; $003BE2
        dc.w    $0002        ; $003BE4
        dc.w    $6A02        ; $003BE6
        dc.w    $4445        ; $003BE8
        dc.w    $BA43        ; $003BEA
        dc.w    $6E2C        ; $003BEC
        dc.w    $9869        ; $003BEE
        dc.w    $0004        ; $003BF0
        dc.w    $6A02        ; $003BF2
        dc.w    $4444        ; $003BF4
        dc.w    $B841        ; $003BF6
        dc.w    $6E20        ; $003BF8
        dc.w    $357C        ; $003BFA
        dc.w    $0001        ; $003BFC
        dc.w    $0000        ; $003BFE
        dc.w    $2559        ; $003C00
        dc.w    $0002        ; $003C02
        dc.w    $3559        ; $003C04
        dc.w    $0006        ; $003C06
        dc.w    $3559        ; $003C08
        dc.w    $000A        ; $003C0A
        dc.w    $3551        ; $003C0C
        dc.w    $000E        ; $003C0E
        dc.w    $257C        ; $003C10
        dc.w    $2229        ; $003C12
        dc.w    $5A24        ; $003C14
        dc.w    $0010        ; $003C16
        dc.w    $4E75        ; $003C18
        dc.w    $43E9        ; $003C1A
        dc.w    $000A        ; $003C1C
        dc.w    $51CF        ; $003C1E
        dc.w    $FFAC        ; $003C20
        dc.w    $357C        ; $003C22
        dc.w    $0000        ; $003C24
        dc.w    $0000        ; $003C26
        dc.w    $4E75        ; $003C28
        dc.w    $F190        ; $003C2A
        dc.w    $09FA        ; $003C2C
        dc.w    $F1F0        ; $003C2E
        dc.w    $F190        ; $003C30
        dc.w    $0A40        ; $003C32
        dc.w    $F1F0        ; $003C34
        dc.w    $EA70        ; $003C36
        dc.w    $03B3        ; $003C38
        dc.w    $FB50        ; $003C3A
        dc.w    $E900        ; $003C3C
        dc.w    $0AF0        ; $003C3E
        dc.w    $0800        ; $003C40
        dc.w    $EA70        ; $003C42
        dc.w    $03B3        ; $003C44
        dc.w    $FB50        ; $003C46
        dc.w    $F190        ; $003C48
        dc.w    $09FA        ; $003C4A
        dc.w    $F1F0        ; $003C4C
        dc.w    $0030        ; $003C4E
        dc.w    $002B        ; $003C50
        dc.w    $0031        ; $003C52
        dc.w    $FFFD        ; $003C54
        dc.w    $FFD7        ; $003C56
        dc.w    $002B        ; $003C58
        dc.w    $FFCE        ; $003C5A
        dc.w    $FFFD        ; $003C5C
        dc.w    $0034        ; $003C5E
        dc.w    $0021        ; $003C60
        dc.w    $0035        ; $003C62
        dc.w    $FFF3        ; $003C64
        dc.w    $FFD3        ; $003C66
        dc.w    $0021        ; $003C68
        dc.w    $FFCA        ; $003C6A
        dc.w    $FFF3        ; $003C6C
        dc.w    $0034        ; $003C6E
        dc.w    $0017        ; $003C70
        dc.w    $0035        ; $003C72
        dc.w    $FFEE        ; $003C74
        dc.w    $FFD3        ; $003C76
        dc.w    $0017        ; $003C78
        dc.w    $FFCA        ; $003C7A
        dc.w    $FFEE        ; $003C7C
        dc.w    $3038        ; $003C7E
        dc.w    $C8C8        ; $003C80
        dc.w    $E948        ; $003C82
        dc.w    $45FB        ; $003C84
        dc.w    $00C8        ; $003C86
        dc.w    $3038        ; $003C88
        dc.w    $C8A0        ; $003C8A
        dc.w    $D078        ; $003C8C
        dc.w    $C89E        ; $003C8E
        dc.w    $47FB        ; $003C90
        dc.w    $0098        ; $003C92
        dc.w    $43F9        ; $003C94
        dc.w    $00FF        ; $003C96
        dc.w    $66DC        ; $003C98
        dc.w    $7201        ; $003C9A
        dc.w    $7403        ; $003C9C
        dc.w    $3341        ; $003C9E
        dc.w    $0000        ; $003CA0
        dc.w    $301B        ; $003CA2
        dc.w    $D05A        ; $003CA4
        dc.w    $3340        ; $003CA6
        dc.w    $0002        ; $003CA8
        dc.w    $335B        ; $003CAA
        dc.w    $0004        ; $003CAC
        dc.w    $301B        ; $003CAE
        dc.w    $905A        ; $003CB0
        dc.w    $3340        ; $003CB2
        dc.w    $0006        ; $003CB4
        dc.w    $43E9        ; $003CB6
        dc.w    $0014        ; $003CB8
        dc.w    $5D4B        ; $003CBA
        dc.w    $51CA        ; $003CBC
        dc.w    $FFE0        ; $003CBE
        dc.w    $7000        ; $003CC0
        dc.w    $600A        ; $003CC2
        dc.w    $7000        ; $003CC4
        dc.w    $3038        ; $003CC6
        dc.w    $C026        ; $003CC8
        dc.w    $6A02        ; $003CCA
        dc.w    $4E75        ; $003CCC
        dc.w    $43F9        ; $003CCE
        dc.w    $00FF        ; $003CD0
        dc.w    $66EC        ; $003CD2
        dc.w    $45F9        ; $003CD4
        dc.w    $0089        ; $003CD6
        dc.w    $5B7E        ; $003CD8
        dc.w    $E940        ; $003CDA
        dc.w    $7203        ; $003CDC
        dc.w    $265A        ; $003CDE
        dc.w    $D7C0        ; $003CE0
        dc.w    $228B        ; $003CE2
        dc.w    $43E9        ; $003CE4
        dc.w    $0014        ; $003CE6
        dc.w    $51C9        ; $003CE8
        dc.w    $FFF4        ; $003CEA
        dc.w    $3238        ; $003CEC
        dc.w    $C026        ; $003CEE
        dc.w    $0C41        ; $003CF0
        dc.w    $0007        ; $003CF2
        dc.w    $6D2C        ; $003CF4
        dc.w    $0C41        ; $003CF6
        dc.w    $0013        ; $003CF8
        dc.w    $6C26        ; $003CFA
        dc.w    $7000        ; $003CFC
        dc.w    $43F9        ; $003CFE
        dc.w    $00FF        ; $003D00
        dc.w    $6128        ; $003D02
        dc.w    $3340        ; $003D04
        dc.w    $0000        ; $003D06
        dc.w    $3340        ; $003D08
        dc.w    $0014        ; $003D0A
        dc.w    $4A78        ; $003D0C
        dc.w    $C04C        ; $003D0E
        dc.w    $660E        ; $003D10
        dc.w    $3340        ; $003D12
        dc.w    $0028        ; $003D14
        dc.w    $4A78        ; $003D16
        dc.w    $C8C8        ; $003D18
        dc.w    $6604        ; $003D1A
        dc.w    $3340        ; $003D1C
        dc.w    $003C        ; $003D1E
        dc.w    $4E75        ; $003D20
        dc.w    $0C41        ; $003D22
        dc.w    $0004        ; $003D24
        dc.w    $6606        ; $003D26
        dc.w    $11FC        ; $003D28
        dc.w    $00BA        ; $003D2A

; --- High-level utility 4 ---
HighLevelUtil4:
        AND.L -(A4),D4        ; $003D2C
        CMPI.W #$0016,D1        ; $003D2E
        BNE $00883D3A        ; $003D32
        MOVE.B #$00BA,$C8A4.W        ; $003D34
        MOVEQ #$01,D0        ; $003D3A
        LEA $00FF6128,A1        ; $003D3C
        MOVE.W D0,$0000(A1)        ; $003D42
        MOVE.W D0,$0014(A1)        ; $003D46
        TST.W $C04C.W        ; $003D4A
        BNE $00883D58        ; $003D4E
        MOVE.W D0,$0028(A1)        ; $003D50
        MOVE.W D0,$003C(A1)        ; $003D54
        RTS        ; $003D58
        dc.w    $43F9        ; $003D5A
        dc.w    $00FF        ; $003D5C
        dc.w    $6980        ; $003D5E
        dc.w    $12BC        ; $003D60
        dc.w    $0009        ; $003D62
        dc.w    $237C        ; $003D64
        dc.w    $0402        ; $003D66
        dc.w    $68F8        ; $003D68

; --- High-level utility 5 ---
HighLevelUtil5:
        BTST #124,D4        ; $003D6A
        MOVE.L $0FBE(A7),D1        ; $003D6E
        BTST #249,A0        ; $003D72
        BSET #192,<EA:3F>        ; $003D76
        MOVE.B #$0009,(A1)        ; $003D7A
        LEA $00FF6990,A1        ; $003D7E
        MOVE.B #$0009,(A1)        ; $003D84
        MOVE.L #$0402C8EC,$0004(A1)        ; $003D88
        MOVE.L #$222F22A2,$0008(A1)        ; $003D90
        RTS        ; $003D98
        dc.w    $31FC        ; $003D9A
        dc.w    $0000        ; $003D9C
        dc.w    $C8AA        ; $003D9E
        dc.w    $5878        ; $003DA0
        dc.w    $C8AC        ; $003DA2
        dc.w    $4E75        ; $003DA4
        dc.w    $0C78        ; $003DA6
        dc.w    $0014        ; $003DA8
        dc.w    $C8AA        ; $003DAA
        dc.w    $6F24        ; $003DAC
        dc.w    $43F9        ; $003DAE
        dc.w    $00FF        ; $003DB0
        dc.w    $69C0        ; $003DB2
        dc.w    $137C        ; $003DB4
        dc.w    $0009        ; $003DB6
        dc.w    $0000        ; $003DB8
        dc.w    $237C        ; $003DBA
        dc.w    $222F        ; $003DBC
        dc.w    $1D4A        ; $003DBE
        dc.w    $0008        ; $003DC0
        dc.w    $31FC        ; $003DC2
        dc.w    $0000        ; $003DC4
        dc.w    $C8AA        ; $003DC6
        dc.w    $11FC        ; $003DC8
        dc.w    $00A6        ; $003DCA
        dc.w    $C8A4        ; $003DCC
        dc.w    $5878        ; $003DCE
        dc.w    $C8AC        ; $003DD0
        dc.w    $4E75        ; $003DD2
        dc.w    $0C78        ; $003DD4
        dc.w    $0014        ; $003DD6
        dc.w    $C8AA        ; $003DD8
        dc.w    $6F2A        ; $003DDA
        dc.w    $23FC        ; $003DDC
        dc.w    $222F        ; $003DDE
        dc.w    $29EE        ; $003DE0
        dc.w    $00FF        ; $003DE2
        dc.w    $69C8        ; $003DE4
        dc.w    $23FC        ; $003DE6
        dc.w    $222F        ; $003DE8
        dc.w    $1716        ; $003DEA
        dc.w    $00FF        ; $003DEC
        dc.w    $6998        ; $003DEE
        dc.w    $31FC        ; $003DF0
        dc.w    $0000        ; $003DF2
        dc.w    $C8AA        ; $003DF4
        dc.w    $11FC        ; $003DF6
        dc.w    $00A7        ; $003DF8
        dc.w    $C8A4        ; $003DFA
        dc.w    $08F8        ; $003DFC
        dc.w    $0004        ; $003DFE
        dc.w    $C30E        ; $003E00
        dc.w    $5878        ; $003E02
        dc.w    $C8AC        ; $003E04
        dc.w    $4E75        ; $003E06
        dc.w    $0C78        ; $003E08
        dc.w    $000A        ; $003E0A
        dc.w    $C8AA        ; $003E0C
        dc.w    $6618        ; $003E0E
        dc.w    $3038        ; $003E10
        dc.w    $C89C        ; $003E12
        dc.w    $11FB        ; $003E14
        dc.w    $003C        ; $003E16
        dc.w    $C8A5        ; $003E18
        dc.w    $0838        ; $003E1A
        dc.w    $0005        ; $003E1C
        dc.w    $C80E        ; $003E1E
        dc.w    $6706        ; $003E20
        dc.w    $11FC        ; $003E22
        dc.w    $0093        ; $003E24
        dc.w    $C8A5        ; $003E26
        dc.w    $0C78        ; $003E28
        dc.w    $0028        ; $003E2A
        dc.w    $C8AA        ; $003E2C
        dc.w    $6F20        ; $003E2E
        dc.w    $31FC        ; $003E30
        dc.w    $0000        ; $003E32
        dc.w    $C8AA        ; $003E34
        dc.w    $31FC        ; $003E36
        dc.w    $0000        ; $003E38
        dc.w    $C8AC        ; $003E3A
        dc.w    $7000        ; $003E3C
        dc.w    $13C0        ; $003E3E
        dc.w    $00FF        ; $003E40
        dc.w    $6980        ; $003E42
        dc.w    $13C0        ; $003E44
        dc.w    $00FF        ; $003E46
        dc.w    $6990        ; $003E48
        dc.w    $13C0        ; $003E4A
        dc.w    $00FF        ; $003E4C
        dc.w    $69C0        ; $003E4E
        dc.w    $4E75        ; $003E50
        dc.w    $8383        ; $003E52
        dc.w    $8499        ; $003E54
        dc.w    $9E99        ; $003E56
        dc.w    $31FC        ; $003E58
        dc.w    $0000        ; $003E5A
        dc.w    $C8AA        ; $003E5C
        dc.w    $5878        ; $003E5E
        dc.w    $C8AC        ; $003E60
        dc.w    $4E75        ; $003E62
        dc.w    $31FC        ; $003E64
        dc.w    $0000        ; $003E66
        dc.w    $C8AA        ; $003E68
        dc.w    $5878        ; $003E6A
        dc.w    $C8AC        ; $003E6C
        dc.w    $13FC        ; $003E6E
        dc.w    $0009        ; $003E70
        dc.w    $00FF        ; $003E72
        dc.w    $6980        ; $003E74
        dc.w    $11FC        ; $003E76
        dc.w    $00C0        ; $003E78
        dc.w    $C8A4        ; $003E7A
        dc.w    $4E75        ; $003E7C
        dc.w    $0C78        ; $003E7E
        dc.w    $0014        ; $003E80
        dc.w    $C8AA        ; $003E82
        dc.w    $6F1A        ; $003E84
        dc.w    $23FC        ; $003E86
        dc.w    $222F        ; $003E88
        dc.w    $038A        ; $003E8A
        dc.w    $00FF        ; $003E8C
        dc.w    $6988        ; $003E8E
        dc.w    $11FC        ; $003E90
        dc.w    $00C1        ; $003E92
        dc.w    $C8A4        ; $003E94
        dc.w    $31FC        ; $003E96
        dc.w    $0000        ; $003E98
        dc.w    $C8AA        ; $003E9A
        dc.w    $5878        ; $003E9C
        dc.w    $C8AC        ; $003E9E
        dc.w    $4E75        ; $003EA0
        dc.w    $0C78        ; $003EA2
        dc.w    $0014        ; $003EA4
        dc.w    $C8AA        ; $003EA6
        dc.w    $6F1A        ; $003EA8
        dc.w    $23FC        ; $003EAA
        dc.w    $222F        ; $003EAC
        dc.w    $002C        ; $003EAE
        dc.w    $00FF        ; $003EB0
        dc.w    $6988        ; $003EB2
        dc.w    $11FC        ; $003EB4
        dc.w    $00C2        ; $003EB6
        dc.w    $C8A4        ; $003EB8
        dc.w    $31FC        ; $003EBA
        dc.w    $0000        ; $003EBC
        dc.w    $C8AA        ; $003EBE
        dc.w    $5878        ; $003EC0
        dc.w    $C8AC        ; $003EC2
        dc.w    $4E75        ; $003EC4
        dc.w    $0C78        ; $003EC6
        dc.w    $0014        ; $003EC8
        dc.w    $C8AA        ; $003ECA
        dc.w    $6F26        ; $003ECC
        dc.w    $23FC        ; $003ECE
        dc.w    $222E        ; $003ED0
        dc.w    $EF3A        ; $003ED2
        dc.w    $00FF        ; $003ED4
        dc.w    $6988        ; $003ED6
        dc.w    $11FC        ; $003ED8
        dc.w    $00C3        ; $003EDA
        dc.w    $C8A4        ; $003EDC
        dc.w    $31FC        ; $003EDE
        dc.w    $0000        ; $003EE0
        dc.w    $C8AA        ; $003EE2
        dc.w    $08F8        ; $003EE4
        dc.w    $0004        ; $003EE6
        dc.w    $C30E        ; $003EE8
        dc.w    $08F8        ; $003EEA
        dc.w    $0004        ; $003EEC
        dc.w    $B4EE        ; $003EEE
        dc.w    $5878        ; $003EF0
        dc.w    $C8AC        ; $003EF2
        dc.w    $4E75        ; $003EF4
        dc.w    $0C78        ; $003EF6
        dc.w    $0005        ; $003EF8
        dc.w    $C8AA        ; $003EFA
        dc.w    $6606        ; $003EFC
        dc.w    $11FC        ; $003EFE
        dc.w    $0098        ; $003F00
        dc.w    $C8A5        ; $003F02
        dc.w    $7000        ; $003F04
        dc.w    $0838        ; $003F06
        dc.w    $0002        ; $003F08
        dc.w    $C8AB        ; $003F0A
        dc.w    $6602        ; $003F0C
        dc.w    $7009        ; $003F0E
        dc.w    $13C0        ; $003F10
        dc.w    $00FF        ; $003F12
        dc.w    $6980        ; $003F14
        dc.w    $0C78        ; $003F16
        dc.w    $003C        ; $003F18
        dc.w    $C8AA        ; $003F1A
        dc.w    $6F0C        ; $003F1C
        dc.w    $13FC        ; $003F1E
        dc.w    $0000        ; $003F20
        dc.w    $00FF        ; $003F22
        dc.w    $6980        ; $003F24
        dc.w    $5878        ; $003F26
        dc.w    $C8AC        ; $003F28
        dc.w    $4E75        ; $003F2A
        dc.w    $4E75        ; $003F2C
        dc.w    $7600        ; $003F2E
        dc.w    $7800        ; $003F30
        dc.w    $4A79        ; $003F32
        dc.w    $00FF        ; $003F34
        dc.w    $6448        ; $003F36
        dc.w    $670A        ; $003F38
        dc.w    $4A68        ; $003F3A
        dc.w    $00C0        ; $003F3C
        dc.w    $6704        ; $003F3E
        dc.w    $7601        ; $003F40
        dc.w    $7802        ; $003F42
        dc.w    $43F9        ; $003F44
        dc.w    $00FF        ; $003F46
        dc.w    $64AC        ; $003F48
        dc.w    $4EBA        ; $003F4A
        dc.w    $0044        ; $003F4C
        dc.w    $7601        ; $003F4E
        dc.w    $7802        ; $003F50
        dc.w    $43F9        ; $003F52
        dc.w    $00FF        ; $003F54
        dc.w    $6178        ; $003F56
        dc.w    $6036        ; $003F58
        dc.w    $7600        ; $003F5A
        dc.w    $7800        ; $003F5C
        dc.w    $4A79        ; $003F5E
        dc.w    $00FF        ; $003F60
        dc.w    $6218        ; $003F62
        dc.w    $670A        ; $003F64
        dc.w    $4A68        ; $003F66
        dc.w    $00C0        ; $003F68
        dc.w    $6704        ; $003F6A
        dc.w    $7601        ; $003F6C
        dc.w    $7802        ; $003F6E
        dc.w    $43F9        ; $003F70
        dc.w    $00FF        ; $003F72
        dc.w    $627C        ; $003F74
        dc.w    $4EBA        ; $003F76
        dc.w    $0018        ; $003F78
        dc.w    $7601        ; $003F7A
        dc.w    $7802        ; $003F7C
        dc.w    $43F9        ; $003F7E
        dc.w    $00FF        ; $003F80
        dc.w    $63A8        ; $003F82
        dc.w    $600A        ; $003F84
        dc.w    $43F9        ; $003F86
        dc.w    $00FF        ; $003F88
        dc.w    $6178        ; $003F8A
        dc.w    $7601        ; $003F8C
        dc.w    $7802        ; $003F8E
        dc.w    $7200        ; $003F90
        dc.w    $7414        ; $003F92
        dc.w    $3341        ; $003F94
        dc.w    $0000        ; $003F96
        dc.w    $3381        ; $003F98
        dc.w    $2000        ; $003F9A
        dc.w    $3038        ; $003F9C
        dc.w    $C00C        ; $003F9E
        dc.w    $6B1E        ; $003FA0
        dc.w    $D040        ; $003FA2
        dc.w    $D040        ; $003FA4
        dc.w    $45F9        ; $003FA6
        dc.w    $0089        ; $003FA8
        dc.w    $59B0        ; $003FAA
        dc.w    $2372        ; $003FAC
        dc.w    $0000        ; $003FAE
        dc.w    $0010        ; $003FB0
        dc.w    $3344        ; $003FB2
        dc.w    $0000        ; $003FB4
        dc.w    $23B2        ; $003FB6
        dc.w    $0000        ; $003FB8
        dc.w    $2010        ; $003FBA
        dc.w    $3383        ; $003FBC
        dc.w    $2000        ; $003FBE
        dc.w    $D3C2        ; $003FC0
        dc.w    $D3C2        ; $003FC2
        dc.w    $3341        ; $003FC4
        dc.w    $0000        ; $003FC6
        dc.w    $3038        ; $003FC8
        dc.w    $C012        ; $003FCA
        dc.w    $6B14        ; $003FCC
        dc.w    $D040        ; $003FCE

; --- High-level utility 6 ---
HighLevelUtil6:
        ADD.W D0,D0        ; $003FD0
        LEA $008959D0,A2        ; $003FD2
        MOVE.L <EA:32>,$0000(A1)        ; $003FD8
        BTST #67,(A0)        ; $003FDC
        BTST #194,D0        ; $003FE0
        MOVE.W D1,$0000(A1)        ; $003FE4
        MOVE.W $C018.W,D0        ; $003FE8
        BMI $00884002        ; $003FEC
        ADD.W D0,D0        ; $003FEE
        ADD.W D0,D0        ; $003FF0
        LEA $008959FC,A2        ; $003FF2
        MOVE.L <EA:32>,$0000(A1)        ; $003FF8
        BTST #67,(A0)        ; $003FFC
        BTST #194,D0        ; $004000

; --- Dual conditional table lookup & write ---
DualTableLookupWrite:
        MOVE.W D1,$0000(A1)        ; $004004
        MOVE.W $C01E.W,D0        ; $004008
        BMI $00884022        ; $00400C
        ADD.W D0,D0        ; $00400E
        ADD.W D0,D0        ; $004010
        LEA $00895A24,A2        ; $004012
        MOVE.L <EA:32>,$0000(A1)        ; $004018
        BTST #68,(A0)        ; $00401C
        BTST #194,D0        ; $004020
        MOVE.W D1,$0000(A1)        ; $004024
        MOVE.W $C024.W,D0        ; $004028
        BMI $00884042        ; $00402C
        ADD.W D0,D0        ; $00402E
        ADD.W D0,D0        ; $004030
        LEA $00895A24,A2        ; $004032
        MOVE.L <EA:32>,$0000(A1)        ; $004038
        BTST #67,(A0)        ; $00403C
        BTST #194,D0        ; $004040
        MOVE.W D1,$0000(A1)        ; $004044
        MOVE.W $C00E.W,D0        ; $004048
        BMI $00884062        ; $00404C
        ADD.W D0,D0        ; $00404E
        ADD.W D0,D0        ; $004050
        LEA $00895A44,A2        ; $004052
        MOVE.L <EA:32>,$0000(A1)        ; $004058
        BTST #68,(A0)        ; $00405C
        BTST #194,D0        ; $004060
        MOVE.W D1,$0000(A1)        ; $004064
        MOVE.W $C010.W,D0        ; $004068
        BMI $00884082        ; $00406C
        ADD.W D0,D0        ; $00406E
        ADD.W D0,D0        ; $004070
        LEA $00895A44,A2        ; $004072
        MOVE.L <EA:32>,$0000(A1)        ; $004078
        BTST #67,(A0)        ; $00407C
        BTST #117,D0        ; $004080
        dc.w    $31FC        ; $004084
        dc.w    $0001        ; $004086
        dc.w    $C048        ; $004088
        dc.w    $3038        ; $00408A
        dc.w    $C07C        ; $00408C
        dc.w    $227B        ; $00408E
        dc.w    $0004        ; $004090
        dc.w    $4ED1        ; $004092
        dc.w    $0088        ; $004094
        dc.w    $40C8        ; $004096
        dc.w    $0088        ; $004098
        dc.w    $412E        ; $00409A
        dc.w    $0088        ; $00409C
        dc.w    $413A        ; $00409E
        dc.w    $0088        ; $0040A0
        dc.w    $4168        ; $0040A2
        dc.w    $0088        ; $0040A4
        dc.w    $417C        ; $0040A6
        dc.w    $0088        ; $0040A8
        dc.w    $41E4        ; $0040AA
        dc.w    $0088        ; $0040AC
        dc.w    $42BA        ; $0040AE
        dc.w    $0088        ; $0040B0
        dc.w    $4300        ; $0040B2
        dc.w    $0088        ; $0040B4
        dc.w    $432E        ; $0040B6
        dc.w    $0088        ; $0040B8
        dc.w    $434A        ; $0040BA
        dc.w    $0088        ; $0040BC
        dc.w    $4390        ; $0040BE
        dc.w    $0088        ; $0040C0
        dc.w    $43BC        ; $0040C2
        dc.w    $0088        ; $0040C4
        dc.w    $43D0        ; $0040C6
        dc.w    $11FC        ; $0040C8
        dc.w    $0001        ; $0040CA
        dc.w    $C800        ; $0040CC
        dc.w    $0C78        ; $0040CE
        dc.w    $FFFF        ; $0040D0
        dc.w    $C050        ; $0040D2
        dc.w    $6606        ; $0040D4
        dc.w    $31FC        ; $0040D6
        dc.w    $0000        ; $0040D8
        dc.w    $C050        ; $0040DA
        dc.w    $13FC        ; $0040DC
        dc.w    $0000        ; $0040DE
        dc.w    $00FF        ; $0040E0
        dc.w    $6960        ; $0040E2
        dc.w    $13FC        ; $0040E4
        dc.w    $0000        ; $0040E6
        dc.w    $00FF        ; $0040E8
        dc.w    $6930        ; $0040EA
        dc.w    $13FC        ; $0040EC
        dc.w    $0000        ; $0040EE
        dc.w    $00FF        ; $0040F0
        dc.w    $6970        ; $0040F2
        dc.w    $45F9        ; $0040F4
        dc.w    $00FF        ; $0040F6
        dc.w    $6754        ; $0040F8
        dc.w    $11FC        ; $0040FA
        dc.w    $00F3        ; $0040FC
        dc.w    $C822        ; $0040FE
        dc.w    $357C        ; $004100
        dc.w    $FFE0        ; $004102
        dc.w    $0004        ; $004104
        dc.w    $357C        ; $004106
        dc.w    $0040        ; $004108
        dc.w    $0006        ; $00410A
        dc.w    $357C        ; $00410C
        dc.w    $F600        ; $00410E
        dc.w    $0008        ; $004110
        dc.w    $257C        ; $004112
        dc.w    $2229        ; $004114
        dc.w    $660C        ; $004116
        dc.w    $0010        ; $004118
        dc.w    $357C        ; $00411A
        dc.w    $0001        ; $00411C
        dc.w    $0000        ; $00411E
        dc.w    $31FC        ; $004120
        dc.w    $0040        ; $004122
        dc.w    $C25C        ; $004124
        dc.w    $5878        ; $004126
        dc.w    $C07C        ; $004128
        dc.w    $4EFA        ; $00412A
        dc.w    $708C        ; $00412C
        dc.w    $11FC        ; $00412E
        dc.w    $0096        ; $004130
        dc.w    $C8A5        ; $004132
        dc.w    $5878        ; $004134
        dc.w    $C07C        ; $004136
        dc.w    $4E75        ; $004138
        dc.w    $45F9        ; $00413A
        dc.w    $00FF        ; $00413C
        dc.w    $6754        ; $00413E
        dc.w    $5078        ; $004140
        dc.w    $C25C        ; $004142
        dc.w    $3038        ; $004144
        dc.w    $C25C        ; $004146
        dc.w    $3540        ; $004148
        dc.w    $0006        ; $00414A
        dc.w    $546A        ; $00414C
        dc.w    $0004        ; $00414E
        dc.w    $066A        ; $004150
        dc.w    $01C0        ; $004152
        dc.w    $0008        ; $004154
        dc.w    $0C40        ; $004156
        dc.w    $0100        ; $004158
        dc.w    $660A        ; $00415A
        dc.w    $5878        ; $00415C
        dc.w    $C07C        ; $00415E
        dc.w    $31FC        ; $004160
        dc.w    $0000        ; $004162
        dc.w    $C8AA        ; $004164
        dc.w    $4E75        ; $004166
        dc.w    $0C78        ; $004168
        dc.w    $003C        ; $00416A
        dc.w    $C8AA        ; $00416C
        dc.w    $660A        ; $00416E
        dc.w    $5878        ; $004170
        dc.w    $C07C        ; $004172
        dc.w    $31FC        ; $004174
        dc.w    $0000        ; $004176
        dc.w    $C8AA        ; $004178
        dc.w    $4E75        ; $00417A
        dc.w    $0C78        ; $00417C
        dc.w    $0015        ; $00417E
        dc.w    $C8AA        ; $004180
        dc.w    $665E        ; $004182
        dc.w    $31F8        ; $004184
        dc.w    $C096        ; $004186
        dc.w    $C07A        ; $004188
        dc.w    $5878        ; $00418A
        dc.w    $C07C        ; $00418C
        dc.w    $33FC        ; $00418E
        dc.w    $0000        ; $004190
        dc.w    $00FF        ; $004192
        dc.w    $6754        ; $004194
        dc.w    $4A38        ; $004196
        dc.w    $FDA9        ; $004198
        dc.w    $6746        ; $00419A
        dc.w    $0C38        ; $00419C
        dc.w    $0001        ; $00419E
        dc.w    $C30C        ; $0041A0
        dc.w    $663E        ; $0041A2
        dc.w    $0838        ; $0041A4
        dc.w    $0007        ; $0041A6
        dc.w    $FDA8        ; $0041A8
        dc.w    $6636        ; $0041AA
        dc.w    $3038        ; $0041AC
        dc.w    $C89C        ; $0041AE
        dc.w    $01F8        ; $0041B0
        dc.w    $EF07        ; $0041B2
        dc.w    $0C38        ; $0041B4
        dc.w    $0002        ; $0041B6
        dc.w    $FDA9        ; $0041B8
        dc.w    $6604        ; $0041BA
        dc.w    $01F8        ; $0041BC
        dc.w    $FEB7        ; $0041BE
        dc.w    $0C38        ; $0041C0
        dc.w    $001F        ; $0041C2
        dc.w    $FEB7        ; $0041C4
        dc.w    $660C        ; $0041C6
        dc.w    $08F8        ; $0041C8
        dc.w    $0006        ; $0041CA
        dc.w    $FEB7        ; $0041CC
        dc.w    $08F8        ; $0041CE
        dc.w    $0007        ; $0041D0
        dc.w    $FEB7        ; $0041D2
        dc.w    $0C38        ; $0041D4
        dc.w    $001F        ; $0041D6
        dc.w    $EF07        ; $0041D8
        dc.w    $6606        ; $0041DA
        dc.w    $08F8        ; $0041DC
        dc.w    $0000        ; $0041DE
        dc.w    $FDA8        ; $0041E0
        dc.w    $4E75        ; $0041E2
        dc.w    $11FC        ; $0041E4
        dc.w    $0001        ; $0041E6
        dc.w    $C800        ; $0041E8
        dc.w    $11FC        ; $0041EA
        dc.w    $00F3        ; $0041EC
        dc.w    $C822        ; $0041EE
        dc.w    $0838        ; $0041F0
        dc.w    $0005        ; $0041F2
        dc.w    $C30E        ; $0041F4
        dc.w    $6604        ; $0041F6
        dc.w    $4EBA        ; $0041F8
        dc.w    $6FBE        ; $0041FA
        dc.w    $4EB9        ; $0041FC
        dc.w    $0088        ; $0041FE
