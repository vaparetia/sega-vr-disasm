...
; ============================================================================
; state4_epilogue — State 4 tail: camera re-DMA + async block copies via cmd $3F
; Tail-jumped from frame_update_orch_005070 (state 4 handler).
;
; Phase 2B: Block copies offloaded to cmd $3F handler (fire-and-forget).
;   1. camera_avg_and_redma: average prev+curr camera, re-DMA → SH2 re-renders
;      with interpolated camera for the second render this frame.
;   2. vr60_comm_trigger: triggers async cmd $3F — copies geometry+sprites from
;      SDRAM to framebuffer, relays COMM3-5 game state, sets COMM1_LO on done.
; V-INT $54 (armed by state 8 epilogue) polls COMM1_LO and swaps frames.
;
; Removed vs Phase 1B:
;   - 2× sh2_send_cmd block copies (52 bytes) → now in cmd $3F handler
;   - Inline frame swap: btst/bclr/bchg (28 bytes) → V-INT $54 handles it
;
; Size: 24 bytes
; ============================================================================
state4_epilogue:
; --- Average camera and trigger second SH2 render ---
        jsr     camera_avg_and_redma(pc)
; --- Fire-and-forget: async block copies + relay via cmd $3F ---
